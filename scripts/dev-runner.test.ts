...
const devServerInput = {
  mode: "dev:server",
  kairoHome: "/tmp/kairo-dev-runner",
  browser: undefined,
...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("resolveOffset", () => {
    it.effect("uses explicit KAIRO_PORT_OFFSET when provided", () =>
      Effect.gen(function* () {
        const result = yield* resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "KAIRO_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("returns structured context for a negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(error.configKey, "KAIRO_PORT_OFFSET");
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("leaves the shared home implicit and disables browser auto-open", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_HOME, undefined);
        assert.equal(env.KAIRO_NO_BROWSER, "1");
      }),
    );
    ...
    it.effect("allows browser auto-open to be explicitly enabled", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_NO_BROWSER, "0");
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_HOME, path.resolve("/tmp/custom-kairo"));
        assert.equal(env.KAIRO_PORT, "4222");
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.KAIRO_NO_BROWSER, "1");
        assert.equal(env.KAIRO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.KAIRO_LOG_WS_EVENTS, "1");
        assert.equal(env.KAIRO_HOST, "0.0.0.0");
    ...
    it.effect("strips inherited service-launcher context", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            KAIRO_SERVICE_LAUNCHER_CONTEXT: '{"childVersion":"9.9.9"}',
            KAIRO_BOOT_SERVICE_UNIT: "kairo.service",
          },
          serverOffset: 0,
          webOffset: 0,
          kairoHome: undefined,
        ...
        assert.equal(env.KAIRO_SERVICE_LAUNCHER_CONTEXT, undefined);
        assert.equal(env.KAIRO_BOOT_SERVICE_UNIT, undefined);
      }),
    );
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            KAIRO_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          kairoHome: undefined,
        ...
        assert.equal(env.KAIRO_MODE, "web");
        assert.equal(env.KAIRO_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_LOG_WS_EVENTS, "0");
      }),
    );

    it.effect("uses custom kairoHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
        ...
        assert.equal(env.KAIRO_HOME, path.resolve("/tmp/my-kairo"));
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_MODE, undefined);
        assert.equal(env.KAIRO_NO_BROWSER, undefined);
        assert.equal(env.KAIRO_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KAIRO_PORT, "13773");
        assert.equal(env.PORT, "5733");
      }),
    );
    ...
    for (const mode of ["dev", "dev:web"] as const) {
      it.effect(`leaves the client backend URLs unset in ${mode} mode`, () =>
        Effect.gen(function* () {
          ...
          assert.equal(env.KAIRO_SINGLE_ORIGIN_DEV, "1");
        }),
      );
    }
    ...
    it.effect("clears the single-origin marker in dev:desktop mode", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev:desktop",
          baseEnv: { KAIRO_SINGLE_ORIGIN_DEV: "1" },
          serverOffset: 0,
          webOffset: 0,
          kairoHome: undefined,
          browser: undefined,
        ...
        assert.equal(env.KAIRO_SINGLE_ORIGIN_DEV, undefined);
    ...
    // --host configures the *backend* (KAIRO_HOST). It must not become Vite's
  ...
  describe("devPortProbeHosts", () => {
    ...
    // actually bind, so --host/KAIRO_HOST has to be probed as well.
    ...
    // Only the backend honours --host/KAIRO_HOST. Vite reads HOST (set for
  ...
  describe("runDevRunnerWithInput", () => {
    it.effect("preserves invalid configuration as the exact cause", () =>
      Effect.gen(function* () {
          ...
          Effect.provide(
            Layer.merge(
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({ env: { KAIRO_PORT_OFFSET: "not-an-integer" } }),
              ),
            ),
          ),
          Effect.flip,
        ...
        assert.deepStrictEqual(error.configKeys, ["KAIRO_PORT_OFFSET", "KAIRO_DEV_INSTANCE"]);
    ...
    // Base-dir precedence (--home-dir > worktree .kairo > ambient KAIRO_HOME)
    ...
    it.effect("ignores an ambient KAIRO_HOME when no home is resolved", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: { KAIRO_HOME: "/home/user/.kairo" },
          serverOffset: 0,
          webOffset: 0,
          kairoHome: undefined,
          browser: undefined,
    ...
    // defaults: an explicit KAIRO_BUNDLED_DEV (even "0") must pass through.
    describe("--share bundled dev default", () => {
          ...
          yield* runDevRunnerWithInput({
            ...
            Effect.provideService(
              ...
              input.ambientBundledDev === undefined
                ? {}
                : { KAIRO_BUNDLED_DEV: input.ambientBundledDev },
            ),
          );
      ...
      it.effect("defaults KAIRO_BUNDLED_DEV=1 for a shared run", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: undefined });
          assert.equal(env?.KAIRO_BUNDLED_DEV, "1");
        }),
      );

      it.effect("keeps an explicit KAIRO_BUNDLED_DEV=0 opt-out", () =>
        Effect.gen(function* () {
          const env = yield* shareSpawnedEnv({ ambientBundledDev: "0" });
          assert.equal(env?.KAIRO_BUNDLED_DEV, "0");
        }),
      );

      it.effect("leaves KAIRO_BUNDLED_DEV unset without --share", () =>
        Effect.gen(function* () {
          let captured: Record<string, string | undefined> | undefined;
          const spawnerLayer = Layer.succeed(
            ChildProcessSpawner.ChildProcessSpawner,
          ...
          assert.equal(captured?.KAIRO_BUNDLED_DEV, undefined);
        }),
      );
    });
    ...
    describe("kairo home precedence", () => {
      const makeWorktree = Effect.acquireRelease(
        Effect.sync(() => {
          const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "kairo-devrunner-"));
      ...
      const spawnedHome = (input: {
        ...
        Effect.gen(function* () {
          ...
          yield* runDevRunnerWithInput({ ...devServerInput, kairoHome: input.kairoHome }).pipe(
            Effect.provide(Layer.mergeAll(emptyConfigLayer, netServiceLayer, spawnerLayer)),
            Effect.provideService(HostProcessPlatform, "linux"),
            Effect.provideService(HostProcessWorkingDirectory, input.cwd),
            Effect.provideService(
              HostProcessEnvironment,
              input.ambientHome === undefined ? {} : { KAIRO_HOME: input.ambientHome },
            ),
          );

          return captured?.KAIRO_HOME;
        });
      ...
      it.effect("prefers an explicit --home-dir over the worktree default", () =>
        Effect.gen(function* () {
          ...
          const home = yield* spawnedHome({
            kairoHome: "/tmp/explicit-home",
            cwd: root,
            ambientHome: "/home/user/.kairo",
          });
      ...
      it.effect("treats a blank --home-dir as unset rather than as a selection", () =>
        Effect.gen(function* () {
          ...
          assert.equal(home, path.join(path.resolve(root), ".kairo"));
        }).pipe(Effect.scoped),
      );

      it.effect("prefers the worktree .kairo over an ambient KAIRO_HOME", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const root = yield* makeWorktree;
          const home = yield* spawnedHome({
      ...
      it.effect("falls back to an ambient KAIRO_HOME outside a worktree", () =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const home = yield* spawnedHome({
            kairoHome: undefined,
          ...
          assert.equal(home, path.resolve("/home/user/.kairo"));
        }),
      );
