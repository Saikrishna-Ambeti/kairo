...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid KAIRO_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric KAIRO_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed KAIRO_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(kairoHome);
    ...
    if (!isDesktopMode && noBrowser !== undefined) {
      output.KAIRO_NO_BROWSER = noBrowser ? "1" : "0";
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read KAIRO_PORT_OFFSET/KAIRO_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KAIRO_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.KAIRO_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  kairoHome: Flag.string("home-dir").pipe(
    Flag.withDescription("Base directory for all Kairo data (equivalent to KAIRO_HOME)."),
    Flag.withFallbackConfig(optionalStringConfig("KAIRO_HOME")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to KAIRO_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("KAIRO_NO_BROWSER")),
  ),
