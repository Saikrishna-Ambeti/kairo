...
import { resolveGitWorktreePath, resolveWorktreeKairoHome } from "@kairo/shared/devHome";
import { HostProcessEnvironment, HostProcessWorkingDirectory } from "@kairo/shared/hostProcess";
...
export const DEFAULT_KAIRO_HOME = Effect.map(Effect.service(Path.Path), (path) =>
  path.join(NodeOS.homedir(), ".kairo"),
);

const MODE_ARGS = {
  dev: [
    "run",
    "--filter=@kairo/contracts",
    "--filter=@kairo/web",
    "--filter=kairo",
    "--parallel",
    "dev",
  ],
  "dev:server": ["run", "--filter=kairo", "dev"],
  "dev:web": ["run", "--filter=@kairo/web", "dev"],
  "dev:desktop": ["run", "--filter=@kairo/desktop", "--filter=@kairo/web", "dev"],
} as const satisfies Record<string, ReadonlyArray<string>>;

type DevMode = keyof typeof MODE_ARGS;
/**
 * `role` matters because only the backend honours `--host`/`KAIRO_HOST`; the
...
export class DevRunnerInvalidPortOffsetError extends Schema.TaggedErrorClass<DevRunnerInvalidPortOffsetError>()(
  ...
  {
    configKey: Schema.Literal("KAIRO_PORT_OFFSET"),
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("KAIRO_PORT_OFFSET"),
  devInstance: optionalStringConfig("KAIRO_DEV_INSTANCE"),
});
...
> {
  if (config.portOffset !== undefined) {
    ...
    return Effect.succeed({
      ...
      source: `KAIRO_PORT_OFFSET=${config.portOffset}`,
    });
  }
  ...
  if (seed) {
    if (/^\d+$/.test(seed)) {
      return Effect.succeed({
        offset: Number(seed),
        source: `numeric KAIRO_DEV_INSTANCE=${seed}`,
      });
    }

    const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
    return Effect.succeed({ offset, source: `hashed KAIRO_DEV_INSTANCE=${seed}` });
  }
...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_KAIRO_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly kairoHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // Precedence (--home-dir > worktree .kairo > ambient KAIRO_HOME) is resolved
    // by the caller; an unset kairoHome here genuinely means "use the default".
    const configuredBaseDir = kairoHome?.trim() || undefined;
    ...
    if (configuredBaseDir !== undefined) {
      output.KAIRO_HOME = resolvedBaseDir;
    } else {
      delete output.KAIRO_HOME;
    }

    // A dev-runner server is never launcher-managed. When the shell that runs
    // this script was itself spawned by the machine's managed kairo service (an
    // agent working inside Kairo), these leak through and the child server
    // fails startup with "The service launcher started a different kairo version"
    // (serviceLauncherClient.ts resolveStartup).
    delete output.KAIRO_SERVICE_LAUNCHER_CONTEXT;
    delete output.KAIRO_BOOT_SERVICE_UNIT;

    if (!isDesktopMode) {
      output.KAIRO_PORT = String(serverPort);
      ...
      if (mode === "dev" || mode === "dev:web") {
        ...
        output.KAIRO_SINGLE_ORIGIN_DEV = "1";
      } else {
        output.VITE_HTTP_URL = `http://localhost:${serverPort}`;
        output.VITE_WS_URL = `ws://localhost:${serverPort}`;
        delete output.KAIRO_SINGLE_ORIGIN_DEV;
      }
    } else {
      output.KAIRO_PORT = String(serverPort);
      ...
      delete output.KAIRO_SINGLE_ORIGIN_DEV;
      delete output.KAIRO_MODE;
      delete output.KAIRO_NO_BROWSER;
      delete output.KAIRO_HOST;
    }

    if (!isDesktopMode && host !== undefined) {
      output.KAIRO_HOST = host;
    }

    if (!isDesktopMode) {
      output.KAIRO_NO_BROWSER = browser === true ? "0" : "1";
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.KAIRO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.KAIRO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.KAIRO_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.KAIRO_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.KAIRO_MODE = "web";
      delete output.KAIRO_DESKTOP_WS_URL;
    }
 ...
 * target reach it there. When `--host`/`KAIRO_HOST` moves the backend onto
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
      ...
      Effect.mapError(
        (cause) =>
          new DevRunnerConfigurationError({
            configKeys: ["KAIRO_PORT_OFFSET", "KAIRO_DEV_INSTANCE"],
            cause,
          }),
      ),
    );
    ...
    // (gitignored) `.kairo` — see @kairo/shared/devHome for why this must
    // outrank an ambient KAIRO_HOME. `--home-dir` still wins.
    const worktreeHome = yield* resolveWorktreeKairoHome(yield* HostProcessWorkingDirectory);
    ...
    const resolvedKairoHome =
      (input.kairoHome?.trim() || undefined) ??
      worktreeHome ??
      (hostEnvironment.KAIRO_HOME?.trim() || undefined);
    const env = yield* createDevRunnerEnv({
      mode: input.mode,
      baseEnv: hostEnvironment,
      serverOffset,
      webOffset,
      kairoHome: resolvedKairoHome,
    ...
    const selectionSuffix =
      serverOffset !== offset || webOffset !== offset
        ...
        : "";
    const baseDir = env.KAIRO_HOME ?? (yield* DEFAULT_KAIRO_HOME);

    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KAIRO_PORT)} webPort=${String(env.PORT)} baseDir=${baseDir}`,
    );
    ...
    if (input.share) {
      if (input.mode === "dev:server") {
        ...
        if (shared) {
          ...
          // a few chunk requests. Only defaulted, so KAIRO_BUNDLED_DEV=0
          // still opts a --share run back out.
          if (env.KAIRO_BUNDLED_DEV === undefined) {
            env.KAIRO_BUNDLED_DEV = "1";
          }
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  kairoHome: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Kairo data directory; runtime state is stored under userdata (equivalent to KAIRO_HOME). Inside a git worktree this defaults to that worktree's own .kairo so dev state stays off the shared home.",
    ),
    Flag.optional,
  ...
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to KAIRO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("KAIRO_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to KAIRO_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("KAIRO_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to KAIRO_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("KAIRO_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to KAIRO_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("KAIRO_PORT")),
  ),
