...
function resolveGitHubPublishConfig(updateChannel: "latest" | "nightly"):
  ...
  | undefined {
  const rawRepo =
    process.env.KAIRO_DESKTOP_UPDATE_REPOSITORY?.trim() ||
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  yield* runCommand(
    ChildProcess.make({
      ...
      shell: process.platform === "win32",
    })`vp exec --filter @kairo/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
    {
