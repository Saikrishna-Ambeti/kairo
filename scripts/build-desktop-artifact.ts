...
import { fromYaml } from "@kairo/shared/schemaYaml";
...
const DESKTOP_APP_ID = "com.kairo.app";
...
export function resourceMonitorExecutableName(platform: typeof BuildPlatform.Type): string {
  return platform === "win" ? "kairo-resource-monitor.exe" : "kairo-resource-monitor";
}
...
interface StagePackageJson {
  ...
  readonly kairoCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Kairo always passes the user's installed Claude executable to the SDK,
...
export class InvalidAppleTeamIdError extends Schema.TaggedError<InvalidAppleTeamIdError>()(
  ...
  override get message(): string {
    return `KAIRO_APPLE_TEAM_ID '${this.teamId}' must be a 10-character Apple Developer Team ID.`;
  }
}
...
export class MissingMacPasskeyProvisioningProfileError extends Schema.TaggedError<MissingMacPasskeyProvisioningProfileError>()(
  ...
  override get message(): string {
    return "KAIRO_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.";
  }
}
...
export class MissingMacPasskeyDomainConfigurationError extends Schema.TaggedError<MissingMacPasskeyDomainConfigurationError>()(
  ...
  override get message(): string {
    return "KAIRO_CLERK_PUBLISHABLE_KEY or KAIRO_CLERK_PASSKEY_RP_DOMAINS is required for signed macOS passkey builds.";
  }
}
...
export class InvalidMacPasskeyPublishableKeyError extends Schema.TaggedError<InvalidMacPasskeyPublishableKeyError>()(
  ...
  override get message(): string {
    return "KAIRO_CLERK_PUBLISHABLE_KEY is invalid.";
  }
}
...
const BuildEnvConfig = Config.all({
  platform: Config.schema(BuildPlatform, "KAIRO_DESKTOP_PLATFORM").pipe(Config.option),
  target: Config.string("KAIRO_DESKTOP_TARGET").pipe(Config.option),
  arch: Config.schema(BuildArch, "KAIRO_DESKTOP_ARCH").pipe(Config.option),
  version: Config.string("KAIRO_DESKTOP_VERSION").pipe(Config.option),
  outputDir: Config.string("KAIRO_DESKTOP_OUTPUT_DIR").pipe(Config.option),
  skipBuild: Config.boolean("KAIRO_DESKTOP_SKIP_BUILD").pipe(Config.withDefault(false)),
  keepStage: Config.boolean("KAIRO_DESKTOP_KEEP_STAGE").pipe(Config.withDefault(false)),
  signed: Config.boolean("KAIRO_DESKTOP_SIGNED").pipe(Config.withDefault(false)),
  verbose: Config.boolean("KAIRO_DESKTOP_VERBOSE").pipe(Config.withDefault(false)),
  mockUpdates: Config.boolean("KAIRO_DESKTOP_MOCK_UPDATES").pipe(Config.withDefault(false)),
  mockUpdateServerPort: Config.string("KAIRO_DESKTOP_MOCK_UPDATE_SERVER_PORT").pipe(Config.option),
  ...
  wslPrebuild: Config.string("KAIRO_DESKTOP_WSL_PREBUILD").pipe(Config.option),
});
...
export const preflightLinuxDesktopBuild = Effect.fn("preflightLinuxDesktopBuild")(function* (
  ...
  const reuseResourceMonitor = yield* Config.boolean("KAIRO_DESKTOP_REUSE_RESOURCE_MONITOR").pipe(
    Config.withDefault(false),
  );
  const reuseCaptureHelpers = yield* Config.boolean(
    "KAIRO_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS",
...
export const stageLinuxCaptureHelper = Effect.fn("stageLinuxCaptureHelper")(function* (input: {
  ...
  const reuseHelpers = yield* Config.boolean("KAIRO_DESKTOP_REUSE_LINUX_CAPTURE_HELPERS").pipe(
    Config.withDefault(false),
  );
  const binaryPath = path.join(
    ...
    rustTarget!,
    `release/kairo-${input.backend}-snap-shot`,
  );
  if (!reuseHelpers) {
  ...
  const executable = path.join(destination, `kairo-${input.backend}-snap-shot`);
  ...
  yield* runCommand(
    ChildProcess.make(
      ...
      [
        ...
        "--output",
        path.join(input.stageResourcesDir, "browser-secret", "kairo-browser-secret"),
      ],
      { cwd: input.repoRoot },
    ),
...
export const resolveGitHubPublishConfig = Effect.fn("resolveGitHubPublishConfig")(function* (
  ...
  const env = yield* Config.all({
    updateRepository: Config.string("KAIRO_DESKTOP_UPDATE_REPOSITORY").pipe(Config.option),
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    artifactName: "Kairo-Code-${version}-${arch}.${ext}",
    ...
    buildConfig.mac = {
      ...
      extendInfo: {
        NSScreenCaptureUsageDescription:
          "Kairo captures the active window when you use the window capture shortcut.",
      },
      protocols: [
        {
          name: "Kairo",
          schemes: ["kairo", "kairo-dev"],
        },
      ],
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // kairo:// OAuth callbacks to the app.
      protocols: [
        {
          name: "Kairo",
          schemes: ["kairo", "kairo-dev"],
...
// node-pty's prebuilds/linux-<arch>/ with a kairo marker the WSL preflight
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  if (input.prebuildPath === undefined) {
    yield* Effect.logWarning(
      "[desktop-artifact] No WSL node-pty prebuild provided (--wsl-prebuild / KAIRO_DESKTOP_WSL_PREBUILD); the packaged WSL backend will not start until a Linux pty.node is bundled.",
    );
    return;
  }
  ...
  yield* fs.writeFileString(path.join(prebuildDir, "kairo-wsl-node-pty.json"), `${markerJson}\n`);
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "kairo-windows-primary-native-probe-",
  });
...
export const validateWindowsPackagedPayload = Effect.fn(
  ...
  if (!(yield* isFile(resourceMonitorPath))) {
    return yield* new WindowsPackagedPayloadValidationError({
      ...
      packagedAppDir,
      missingFiles: ["resource-monitor/kairo-resource-monitor.exe"],
    });
  }
    ...
    const requiredMembers = [
      ...
      ...(wslArch === undefined
        ...
        : [
            ...
            `node_modules/node-pty/prebuilds/linux-${wslArch}/kairo-wsl-node-pty.json`,
          ]),
    ];
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `kairo-desktop-${options.platform}-stage-`,
  });
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    kairoCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Kairo desktop build",
    author: "Kairo Tools",
  ...
  if (shouldUseHdiutilDmgbuildShim(options.platform, hostPlatform, NodeOS.release(), buildEnv)) {
  ...
  yield* runCommand(
    ...
    {
      label: `vp exec --filter @kairo/desktop -- electron-builder --projectDir ${stageAppDir} ${platformConfig.cliFlag} --${options.arch} --publish never`,
      verbose: options.verbose,
    },
  );
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  platform: Flag.choice("platform", BuildPlatform.literals).pipe(
    Flag.withDescription("Build platform (env: KAIRO_DESKTOP_PLATFORM)."),
    Flag.optional,
  ),
  target: Flag.string("target").pipe(
    Flag.withDescription(
      "Artifact target, for example dmg/AppImage/nsis (env: KAIRO_DESKTOP_TARGET).",
    ),
    Flag.optional,
  ),
  arch: Flag.choice("arch", BuildArch.literals).pipe(
    Flag.withDescription("Build arch, for example arm64/x64/universal (env: KAIRO_DESKTOP_ARCH)."),
    Flag.optional,
  ),
  buildVersion: Flag.string("build-version").pipe(
    Flag.withDescription("Artifact version metadata (env: KAIRO_DESKTOP_VERSION)."),
    Flag.optional,
  ),
  outputDir: Flag.string("output-dir").pipe(
    Flag.withDescription("Output directory for artifacts (env: KAIRO_DESKTOP_OUTPUT_DIR)."),
    Flag.optional,
  ),
  skipBuild: Flag.boolean("skip-build").pipe(
    Flag.withDescription(
      "Skip `vp run build:desktop` and use existing dist artifacts (env: KAIRO_DESKTOP_SKIP_BUILD).",
    ),
    Flag.optional,
  ),
  keepStage: Flag.boolean("keep-stage").pipe(
    Flag.withDescription("Keep temporary staging files (env: KAIRO_DESKTOP_KEEP_STAGE)."),
    Flag.optional,
  ),
  signed: Flag.boolean("signed").pipe(
    Flag.withDescription(
      "Enable signing/notarization discovery; Windows uses Azure Trusted Signing (env: KAIRO_DESKTOP_SIGNED).",
    ),
    Flag.optional,
  ),
  verbose: Flag.boolean("verbose").pipe(
    Flag.withDescription("Stream subprocess stdout (env: KAIRO_DESKTOP_VERBOSE)."),
    Flag.optional,
  ),
  mockUpdates: Flag.boolean("mock-updates").pipe(
    Flag.withDescription("Enable mock updates (env: KAIRO_DESKTOP_MOCK_UPDATES)."),
    Flag.optional,
  ),
  mockUpdateServerPort: Flag.integer("mock-update-server-port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Mock update server port (env: KAIRO_DESKTOP_MOCK_UPDATE_SERVER_PORT)."),
    Flag.optional,
  ),
  wslPrebuild: Flag.string("wsl-prebuild").pipe(
    Flag.withDescription(
      "Path to a prebuilt Linux node-pty (pty.node) for the target arch, staged for the WSL backend (env: KAIRO_DESKTOP_WSL_PREBUILD).",
    ),
    Flag.optional,
  ),
}).pipe(
  Command.withDescription("Build a desktop artifact for Kairo."),
