...
const makeWindowsPayloadFixture = Effect.fn("test.makeWindowsPayloadFixture")(function* (input: {
  ...
  const tempDir = yield* fs.makeTempDirectoryScoped({
    prefix: "kairo-windows-payload-test-",
  });
  ...
  yield* fs.writeFileString(
    path.join(resourcesDir, "resource-monitor/kairo-resource-monitor.exe"),
    "monitor",
  );
  const appExecutableName = "kairo.exe";
  ...
  if (input.wslRuntime !== undefined) {
    ...
    yield* fs.writeFileString(
      path.join(linuxPrebuildDir, "kairo-wsl-node-pty.json"),
      '{"arch":"x64"}',
    );
...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                KAIRO_DESKTOP_UPDATE_REPOSITORY: "Saikrishna-Ambeti/kairo",
              },
            }),
          ),
        ),
      ...
      const nightlyConfig = yield* resolveGitHubPublishConfig("nightly").pipe(
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                GITHUB_REPOSITORY: "Saikrishna-Ambeti/kairo",
              },
            }),
          ),
        ),
  ...
  it.effect("omits update feeds for pull request preview builds", () =>
    Effect.gen(function* () {
      ...
      Effect.provide(
        ConfigProvider.layer(
          ConfigProvider.fromEnv({ env: { GITHUB_REPOSITORY: "pingdotgg/kairo" } }),
        ),
      ),
    ),
  );
  ...
  it.effect("applies platform-specific packaging to the build config", () =>
    Effect.gen(function* () {
      ...
      assert.deepStrictEqual(mac.dmg, {
        title: "Kairo (Alpha) 1.2.3 Installer",
      ...
      // entry advertises MimeType=x-scheme-handler/kairo; for OAuth deep links.
    ...
    () =>
      Effect.scoped(
        Effect.gen(function* () {
          ...
          const path = yield* Path.Path;
          const tempDir = yield* fs.makeTempDirectoryScoped({
            prefix: "kairo-windows-architecture-test-",
          });
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const repoRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "kairo-resource-monitor-cache-test-",
        });
        const binaryPath = path.join(
          repoRoot,
          "native/resource-monitor/target/x86_64-unknown-linux-gnu/release/kairo-resource-monitor",
        );
        ...
        yield* stageResourceMonitor({
          ...
          Effect.provide(
            ConfigProvider.layer(
              ConfigProvider.fromEnv({
                env: { KAIRO_DESKTOP_REUSE_RESOURCE_MONITOR: "true" },
              }),
            ),
          ),
        );
        ...
        assert.equal(
          yield* fs.readFileString(
            path.join(stageResourcesDir, "resource-monitor/kairo-resource-monitor"),
          ),
          "cached monitor",
        );
      }),
  ...
  it.effect("reports missing Windows toolchain capabilities before building", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-windows-preflight-" });
              ...
              ConfigProvider.layer(
                ConfigProvider.fromEnv({
                  env: {
                    ...
                    KAIRO_DESKTOP_REUSE_RESOURCE_MONITOR: "true",
                  },
                }),
              ),
            ),
  ...
  it.effect("rejects a PATH-discovered Python executable that is not Python 3", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-python2-preflight-" });
  ...
  it.effect("builds and stages native capture helpers for each Linux architecture", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const path = yield* Path.Path;
        const repoRoot = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-kde-stage-test-" });
        ...
        for (const backend of ["kde", "hyprland"] as const) {
          for (const [arch, target] of [
            ...
            const binary = path.join(
              ...
              target,
              `release/kairo-${backend}-snap-shot`,
            );
            ...
            const installed = path.join(
              stageResourcesDir,
              `${backend}-capture/kairo-${backend}-snap-shot`,
            );
  ...
  // The fixture's kairo.exe is a text placeholder, not an executable. These
  ...
  it.effect("builds the Linux browser secret helper for a concrete architecture", () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(helper.args.slice(-4), [
        ...
        "x64",
        "--output",
        path.join("/stage/resources", "browser-secret", "kairo-browser-secret"),
      ]);
    }).pipe(
      Effect.provide(
        Layer.mergeAll(
    ...
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const resourceMonitorPath = path.join(
          fixture.packagedAppDir,
          "resources/resource-monitor/kairo-resource-monitor.exe",
        );
        ...
        assert.deepStrictEqual(resourceMonitorError.missingFiles, [
          "resource-monitor/kairo-resource-monitor.exe",
        ]);
      }),
    ),
  );
  ...
  it.effect("rejects a sidecar whose extracted server bundle cannot resolve", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fixture = yield* makeWindowsPayloadFixture({
          copyUnpackedNatives: true,
          serverEntrySource: 'import "kairo-deliberately-missing-package";\n',
        });
        const error = yield* validateWindowsPackagedPayload({
          stageDistDir: fixture.stageDistDir,
          appExecutableName: fixture.appExecutableName,
        ...
        assert.include(error.output, "kairo-deliberately-missing-package");
      }),
    ).pipe(Effect.provideService(HostProcessPlatform, "linux")),
  );
  ...
  it.effect("fails clearly when the selected DMG background source is missing", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const fs = yield* FileSystem.FileSystem;
        const stageResourcesDir = yield* fs.makeTempDirectoryScoped({
          prefix: "kairo-dmg-background-missing-",
        });
  ...
  it("derives macOS passkey signing configuration from the Clerk publishable key", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      ...
      KAIRO_MACOS_PROVISIONING_PROFILE: "/tmp/kairo.provisionprofile",
      KAIRO_CLERK_PUBLISHABLE_KEY: `pk_test_${btoa("example.clerk.accounts.dev$")}`,
    });
    ...
    assert.deepStrictEqual(configuration, {
      ...
      provisioningProfilePath: "/tmp/kairo.provisionprofile",
    });
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    const configuration = resolveMacPasskeySigningConfiguration({
      KAIRO_APPLE_TEAM_ID: "ABC1234567",
      KAIRO_MACOS_PROVISIONING_PROFILE: "/tmp/kairo.provisionprofile",
      KAIRO_CLERK_PASSKEY_RP_DOMAINS:
    ...
    assert.include(entitlements, "<string>ABC1234567.com.kairo.app</string>");
  ...
  it("rejects incomplete macOS passkey signing configuration", () => {
    ...
    const missingProfileError = captureError({
      KAIRO_APPLE_TEAM_ID: "ABC1234567",
      KAIRO_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev",
    });
    assert.instanceOf(missingProfileError, MissingMacPasskeyProvisioningProfileError);
    assert.equal(
      missingProfileError.message,
      "KAIRO_MACOS_PROVISIONING_PROFILE must point to an Associated Domains provisioning profile.",
    );
    ...
    const invalidDomainError = captureError({
      KAIRO_APPLE_TEAM_ID: "ABC1234567",
      KAIRO_MACOS_PROVISIONING_PROFILE: "/tmp/kairo.provisionprofile",
      KAIRO_CLERK_PASSKEY_RP_DOMAINS: unsafeDomain,
    });
    ...
    assert.throws(
      () =>
        resolveMacPasskeySigningConfiguration({
          KAIRO_APPLE_TEAM_ID: "ABC1234567",
          KAIRO_MACOS_PROVISIONING_PROFILE: "/tmp/kairo.provisionprofile",
          KAIRO_CLERK_PASSKEY_RP_DOMAINS: "example.clerk.accounts.dev:8443",
        }),
    ...
    const invalidPublishableKeyError = captureError({
      KAIRO_APPLE_TEAM_ID: "ABC1234567",
      KAIRO_MACOS_PROVISIONING_PROFILE: "/tmp/kairo.provisionprofile",
      KAIRO_CLERK_PUBLISHABLE_KEY: "pk_test_%",
    });
    ...
    assert.equal(invalidPublishableKeyError.message, "KAIRO_CLERK_PUBLISHABLE_KEY is invalid.");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "com.kairo.app");
  ...
  it("stages the resource monitor as an external executable resource", () => {
    ...
    assert.equal(resourceMonitorExecutableName("mac"), "kairo-resource-monitor");
    assert.equal(resourceMonitorExecutableName("win"), "kairo-resource-monitor.exe");
  });
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const stageRoot = yield* fs.makeTempDirectoryScoped({
          prefix: "kairo-wsl-runtime-archive-",
        });
  ...
  it.effect("ships only Linux runtime members in the WSL archive", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const spawner = yield* ChildProcessSpawner.ChildProcessSpawner;
        const root = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-wsl-runtime-members-" });
...
it.effect.skipIf(!symlinksSupported)("rebases packaged links into the isolated tree", () =>
  Effect.gen(function* () {
    const fs = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const root = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-copy-symlinks-" });
