...
Load this reference only when inspecting or seeding local Kairo state directly.
...
```bash
node apps/server/scripts/kairo-sqlite-state.ts query \
  --base-dir <base-dir> \
...
```bash
node apps/server/scripts/kairo-sqlite-state.ts exec \
  --base-dir <base-dir> \
  --file /tmp/kairo-seed.sql
```
