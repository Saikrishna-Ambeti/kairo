---
name: test-kairo-mobile
description: Launch and test Kairo Mobile on an iOS Simulator or Android Emulator against disposable local Kairo environments, including Metro and dev-client reuse, native rebuild decisions, per-client pairing, seeded projects, semantic UI control, screenshots, and iOS serve-sim streaming. Use after mobile UI or native changes, when reproducing phone or tablet behavior, pairing an emulator to isolated state, or verifying mobile behavior on macOS, Linux, or Windows.
---

# Test Kairo Mobile

Run one focused, end-to-end mobile verification pass against disposable Kairo state. Use the sibling [`test-kairo-app`](../test-kairo-app/SKILL.md) skill as the detailed reference for pairing-token semantics and SQLite fixtures.
...
- App: `Kairo Dev`
- Bundle/package identifier: `com.kairo.app.dev`
- URL scheme: `kairo-dev`
...
## Start one disposable Kairo environment

Run backend commands from the repository root. Use the ignored, worktree-local `.kairo` directory or create a fresh directory with the host OS's temporary-directory mechanism. An explicit base directory stores state in `<base-dir>/userdata`; never point testing at shared `~/.kairo` state.
...
Use direct SQLite mutation only for disposable projection fixtures. Follow `test-kairo-app` and stop the backend before writing.
...
1. Inspect any process on the intended Metro port and its `/status` response. Reuse it only when it is healthy, belongs to this worktree, and matches `APP_VARIANT=development`, `--dev-client`, and scheme `kairo-dev`.
...
- Workspace: `<repo>/apps/mobile/ios/KairoDev.xcworkspace`
- Scheme: `KairoDev`
- Configuration: `Debug`
- Simulator ID: the selected UDID
- Bundle ID: `com.kairo.app.dev`

Check the installed client with:

```bash
xcrun simctl get_app_container <simulator-udid> com.kairo.app.dev app
...
```bash
adb -s <emulator-serial> shell pm path com.kairo.app.dev
...
```bash
.agents/skills/test-kairo-mobile/scripts/pair-client.sh \
...
```text
kairo-dev://connections/new?pairingUrl=<encoded-pairing-url>&autoConnect=1
```
...
Use `snapshot_ui` and current element references from XcodeBuildMCP for taps and typing. Stream the same UDID through `ios-simulator-browser` so the user can watch in Kairo when the host supports it. Use the stream as a visual feed rather than a reason to switch to fragile browser coordinates.
...
3. Remove the disposable environment from Kairo Dev.
