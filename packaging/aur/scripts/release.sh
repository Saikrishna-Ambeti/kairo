...
version="${tag#v}"
pkgver="${version//-/_}"
asset_name="Kairo-Code-${version}-x86_64.AppImage"
...
cd "$aur_dir"
git rm --ignore-unmatch LICENSE .upstream-commit kairo-icon.png
git config user.name 'kairo-ci'
git config user.email 'kairo-ci@users.noreply.github.com'
git add -A
