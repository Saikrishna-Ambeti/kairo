...
```bash
export KAIRO_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export KAIRO_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export KAIRO_OTLP_SERVICE_NAME=kairo-local
```

Optional:

```bash
export KAIRO_TRACE_MIN_LEVEL=Info
export KAIRO_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `KAIRO_OTLP_*`.

macOS app bundle example:

```bash
KAIRO_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
KAIRO_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
KAIRO_OTLP_SERVICE_NAME=kairo-desktop \
"/Applications/Kairo.app/Contents/MacOS/Kairo"
```
...
- `kairo_rpc_request_duration`
- `kairo_orchestration_command_duration`
...
`kairo_orchestration_command_ack_duration` measures:
...
1. Check `kairo_orchestration_command_ack_duration` by `commandType`.
...
- `KAIRO_OTLP_TRACES_URL` was not set
...
- `KAIRO_TRACE_FILE`: override trace file path
- `KAIRO_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `KAIRO_TRACE_MAX_FILES`: rotated file count, default `10`
- `KAIRO_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `KAIRO_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `KAIRO_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `KAIRO_OTLP_TRACES_URL`: OTLP trace endpoint
- `KAIRO_OTLP_METRICS_URL`: OTLP metric endpoint
- `KAIRO_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `KAIRO_OTLP_SERVICE_NAME`: service name, default `kairo-server`
