...
Flags go directly after the task name, for example `vp run dev --home-dir /tmp/kairo-dev`.
Add `--browser` to open a browser automatically.

### State and ports

Linked worktrees default to their own `.kairo/userdata`, even when `KAIRO_HOME` is set.
The main checkout defaults to `~/.kairo/dev/userdata`. An explicit `--home-dir` wins in both cases.
Never run a development server against the live `~/.kairo/userdata`.
...
but occupied ports can shift them. `KAIRO_PORT_OFFSET` or `KAIRO_DEV_INSTANCE` can select a
...
`KAIRO_BUNDLED_DEV=0` opts out when debugging bundler differences. Two reload traps matter
