...
- `kairo-code-relay-traces-prod`, the OpenTelemetry trace dataset shared by the Worker, mobile app, and
  first-party relay clients
- `kairo-code-relay-otel-ingest-prod`, the dataset-scoped Worker ingest token
- `kairo-code-mobile-otel-ingest-prod`, the dataset-scoped mobile ingest token
- `kairo-code-relay-client-otel-ingest-prod`, the dataset-scoped first-party relay-client ingest token
- `kairo-code-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`kairo-relay-traces-dev-julius` for a personal stage.
