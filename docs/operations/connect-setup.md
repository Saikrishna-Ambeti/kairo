...
Deployment and client configuration for Kairo Connect. The [architecture note](../internals/kairo-connect.md)
...
Development uses `kairo-dev://app`; production uses `kairo://app`. Update the array with
...
| Variant     | Callback                                      |
| ----------- | --------------------------------------------- |
| Development | `clerk://com.kairo.app.dev.callback`     |
| Preview     | `clerk://com.kairo.app.preview.callback` |
| Production  | `clerk://com.kairo.app.callback`         |

Preserve existing entries. These callbacks are separate from the `kairo-dev` / `kairo-preview` / `kairo` navigation schemes. A private development build using the production Clerk key still needs its development callback allowed by that instance's administrator; rebuilding the same package does not change the allowlist.

## Desktop passkeys

For a production macOS app with bundle ID `com.kairo.app`:
...
4. Check `https://<frontend-api>/.well-known/apple-app-site-association`. Its
   `webcredentials.apps` must include `<TEAM_ID>.com.kairo.app`.
