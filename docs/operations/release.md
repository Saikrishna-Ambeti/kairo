...
- Publishes the CLI package (`apps/server`, npm package `kairo`) with OIDC trusted publishing from the same workflow file:
...
## Kairo Connect relay deployment
...
so `AXIOM_ORG_ID` must accompany `AXIOM_TOKEN`. The `prod` stage owns the retained Neon project and
default database. Local personal stages provision isolated branches from it and are never deployed by CI.
...
```sh
vp run --filter kairo-relay deploy -- --stage "$USER" --env-file .env.local
```
...
The job looks up the `kairo-marketing` project using the existing `VERCEL_TOKEN`
...
- `KAIRO_WEB_ROUTER_URL`: defaults to `https://app.kairo.codes`.
- `KAIRO_WEB_LATEST_DOMAIN`: defaults to `latest.app.kairo.codes`.
- `KAIRO_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.kairo.codes`.

Required Vercel domains:

- `app.kairo.codes`: the router domain users open, updated by stable releases.
- `latest.app.kairo.codes`: channel alias updated by stable releases.
- `nightly.app.kairo.codes`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__kairo/channel?channel=latest` or
`/__kairo/channel?channel=nightly`; the router stores the
`kairo_web_channel` cookie and rewrites future requests on `app.kairo.codes` to
...
through `/__kairo/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.kairo.codes` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `kairo`) to the `nightly` npm dist-tag using the same nightly version.
- Does not commit version bumps back to `main`.

## Server self-update release invariant

Connected servers update to the client's exact version, not to an npm dist-tag. Every released
desktop or hosted client version must therefore have a matching `kairo@<version>` package available on
...
For a release smoke test, confirm `npm view kairo@<version> version` returns the expected version, then
...
verify that the update stops before restart and run `npx kairo@<version> service update` once on the
...
- Repository slug source:
  - `KAIRO_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
...
and extracts that archive into `~/.kairo/wsl-runtime/sha256-<archive-digest>` inside
...
script temporarily prepares the `kairo` package, then runs `vp pm publish --filter kairo ...` from the
repository root so workspace publish configuration is applied correctly.

Checklist:

1. Confirm npm org/user owns package `kairo` (or rename package first if needed).
...
`v0.0.0-test.1`, classifies the run as the stable channel. It publishes `kairo` with npm dist-tag
`latest`, creates a real GitHub Release, aliases the hosted app to `latest.app.kairo.codes` and
`app.kairo.codes`, and can commit a version bump to `main` in the finalize job. Do not push a test tag
...
2. Create an explicit App ID for `com.kairo.app` and enable Associated Domains.
...
10. Complete the Clerk Native API and AASA setup in [Kairo Connect setup](./connect-setup.md#desktop-passkeys).
...
- macOS build unsigned when expected signed:
  ...
  - Confirm the provisioning profile belongs to `APPLE_TEAM_ID.com.kairo.app` and includes
    Associated Domains.
