...
Shift keys together. Kairo attaches the image to your draft and brings itself forward. If no thread
is open it starts a draft in the current project.

Pressing the shortcut while Kairo is in front captures Kairo itself.
...
a modifier pair such as Command+Command or Ctrl+Ctrl, or a key chord. Kairo refuses shortcuts
...
terminal contents. If an app is slow to answer, Kairo attaches the screenshot without the data
rather than waiting.
...
**GNOME.** Install the bundled **Kairo SnapShots** extension during setup. It is installed
...
shortcut does not fire, check Kairo under **System Settings** > **Keyboard** > **Shortcuts**. If a
...
**Review changes**. Kairo shows the exact change it will make to your Hyprland config. **Save
...
**Niri.** Choose a shortcut and select **Review changes**. Kairo shows the exact binding it will
...
**Other Wayland desktops.** Kairo uses the desktop's screenshot portal when it can capture the
