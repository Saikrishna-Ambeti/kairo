...
and choose a browser under **Import from**. The import copies cookies into a Kairo browser
...
On Linux, Chromium-based browsers use your desktop keyring to protect their cookies. Kairo
includes the keyring reader; no separate command-line tool is needed. Allow the desktop unlock
prompt if one appears. If the keyring cannot be accessed, Kairo reports that failure when no
