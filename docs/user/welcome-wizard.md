...
Kairo shows a setup flow when you open a new installation or connect to the
hosted app for the first time. Existing workspaces skip this flow.

## Connect your computers

Select one or more computers to set up. If you opened Kairo directly from a
...
- **Kairo Connect** connects computers that are signed in to your account. Run
  `npx kairo connect` on each computer you want to add, then start Kairo or run
  `npx kairo serve` so the computer stays available.
- **Add a computer** connects directly to a server on your network or tailnet.
  Start the server with `npx kairo serve`, then run `npx kairo pair --tailscale` and
  paste the pairing link. You can also run `npx kairo serve --host <address>` and
  use `npx kairo pair` when the server is already reachable on your network.

Saved computers and computers discovered through Kairo Connect are selected by
...
If Kairo cannot confirm the workspace during startup, the setup flow shows
**Still connecting** instead of opening the app. Select **Reload** to try again.

If Kairo cannot read your saved settings, it shows **Could not read settings**.
Select **Retry** after storage becomes available. Setup does not replace
unreadable settings with defaults.

## Check your agents

Kairo checks each selected computer for Claude Code and Codex. If an agent is
...
Kairo finds directories that Claude Code or Codex has used. Git repositories
...
A large or malformed history can reach the scan limit. Kairo keeps the
...
30 days. You can continue those conversations in Kairo.

Conversation import is best effort. Kairo keeps the first user prompt and the
...
conversation so Kairo does not remove user text. It reads one conversation at
