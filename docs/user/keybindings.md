...
The same configuration lives in `~/.kairo/userdata/keybindings.json` on the machine running the
server, if you prefer editing it directly. Kairo writes the built-in defaults into that file on
