...
`~/.kairo/userdata/keybindings.json` by default. You can edit this file directly.
...
Kairo creates the file with its defaults and adds new defaults on later startups.
...
Invalid rules are ignored; if the file cannot be parsed, Kairo uses defaults.
...
`chat.new`. The web and desktop thread header no longer includes the project script button
or its dropdown. Project scripts remain addressable as `script.{id}.run`, for example `script.test.run`.
