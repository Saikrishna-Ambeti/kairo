...
Kairo runs Google's official Antigravity ACP agent on your selected environment.
...
**Sign in with Google** and complete the browser sign-in. Wait for Kairo to confirm
...
hostname. Only that Kairo sign-in session can finish the attempt. If it expires,
...
Use Antigravity's native `/plan` command for planning. Kairo's separate Plan mode
...
Kairo keeps conversation history and file diffs, but Antigravity cannot rewind
...
Put project skills in `.agents/skills`. Kairo also reads `.gemini/skills` and the
