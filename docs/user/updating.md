# Keeping Kairo in Sync

The Kairo web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Kairo shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                                                                                                                                                                                                                              |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Update server**          | Available for the Kairo Linux background service and for servers run by a current Kairo desktop app. Select the button and leave Kairo open while it downloads, installs, restarts, and reconnects. For desktop-app servers this closes and relaunches the desktop app on that machine. If installation fails, the desktop app stays open and reconnects to its server. |
| **Update the desktop app** | Shown for desktop apps that predate remote updates. Open the Kairo desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                                                                                                                                                               |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Kairo server, and relaunch it with the copied command and any startup options you normally use.                                                                                                                                                                                               |

The available action depends on how that server was started. Kairo does not update connected
servers silently in the background.

An older background-service launcher may ask you to run the exact
`npx kairo@<version> service update` command on the server machine. That one local update installs the
...
**Copy update command** gives you `npx kairo@<client-version>`, which relaunches the server directly
at the matching version. Add whatever startup options you normally use.

If the server instead runs as the Kairo background service, update the service on the host and
pin the same version:

```sh
npx kairo@<client-version> service update
```

`service update` installs the version of the CLI that invoked it, so `npx kairo@latest service update`
...
See [Running Kairo in the Background](./background-service.md) for install, status, and removal
commands.
...
3. For a command-line server, relaunch it with `npx kairo@<client-version>`, replacing
