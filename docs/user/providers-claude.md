...
This guide is for people who want to use more than one Claude setup in Kairo. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Kairo](./install.md).
...
In Kairo Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Kairo uses Claude Code's normal config directory.

When you set this field, Kairo points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Kairo
...
Kairo looks for Claude skills in the Claude config directory's `skills` folder and
...
`~/.claude_personal_home/.claude`, which is not where Kairo looks.

Then add another Claude provider in Kairo:
...
Kairo only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Kairo keeps separate config directories isolated
...
Add or edit a Claude provider in Kairo Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Kairo stores the value as a server secret and does not
...
Kairo does not need a special Claude Code Router provider. Treat the router as a Claude
