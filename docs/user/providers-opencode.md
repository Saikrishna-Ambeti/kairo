...
Kairo requires OpenCode 1.14.19 or newer, including when you connect an existing
OpenCode server.

## Local or external server

Leave **Server URL** empty to let Kairo start OpenCode locally. A password in
provider settings applies to both that server and Kairo's connection. With no
...
settings. Kairo uses only that configured password for an external server; it
...
restart before Kairo can see configuration changes.
