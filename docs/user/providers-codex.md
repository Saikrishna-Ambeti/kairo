...
This guide is for people who want to use more than one Codex account in Kairo. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install Kairo](./install.md).
...
issue. Kairo uploads the thread and Codex logs to OpenAI and shows a thread ID that you can copy
...
reports them. If Codex does not report either value, Kairo leaves it out instead of using the
...
When a Codex tool needs access to an app such as Safari, Kairo shows the app name and asks for
...
- both accounts can see the same Kairo/Codex sessions
...
In Kairo Settings, name it something obvious:
...
In Kairo Settings, add another Codex provider:
...
Kairo shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Kairo treats it
