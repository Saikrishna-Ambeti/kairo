...
On web and desktop, choose **Create theme** to adjust a palette, or import a Kairo or VS Code
...
main local environment. app.kairo.codes and additional connections do not use them.
...
takes precedence. If the server stops publishing the selected theme, Kairo falls back to its
standard theme.
...
choosing another theme afterward sticks until the next `kairo theme set`. Run the command again to
reapply it, even if the name is unchanged.

`kairo theme clear` removes the default without changing anyone's current theme. `kairo theme show` lists
the default and published themes.

### Publish a theme

Save a theme exported from Kairo into `~/.kairo/userdata/themes/` on the server, or the `themes`
directory under your custom state directory. The filename supplies the theme ID: `nightfall.json`
can be selected with `kairo theme set nightfall`. Keep the filename stable when updating its colors.
...
Set `appearance` to `light` or `dark` and supply hex colors for `canvas` and `accent`. Kairo
