...
Kairo from other apps through the system share sheet. Mobile keeps a local copy of each draft
attachment, so you can still preview it and queue messages while offline. Uploads resume when
you reconnect. Drafts and queued messages survive app restarts; signing out of Kairo Connect keeps
...
Kairo remembers the last provider, model, and model options you selected and reuses that
...
Model options shown as provider defaults remain display values until you choose them in Kairo.
Kairo only sends options you selected explicitly, so an unset reasoning level or service tier can
...
readable and Kairo shows a warning.
...
On web, desktop, and mobile, select a link to an image or video to open it inside Kairo.
Workspace image and video links open the file viewer. Links to media outside the workspace
open a media preview.
Videos opened from the file explorer or a file-viewer tab also play inside Kairo. They
...
Kairo serves the original file without adding it to attachment storage. If that file is moved
...
runs in an isolated frame with no access to your Kairo session. On desktop, the integrated
...
project exists there. If it does not, Kairo selects another environment that has the project.
...
and attachments. Kairo deletes the local audio file after transcription or cancellation. It sends
...
also reports that skill as a native slash command, Kairo hides the duplicate native entry and keeps
the `/skill:Skill Name` label.

A skill token runs the skill wherever it sits in your message. Kairo sends it to each provider in
...
them only there. Kairo's own commands, such as `/model` and `/plan`, and skills stay available on
any line.
