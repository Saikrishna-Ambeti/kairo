...
default preserves explicit project overrides. Workspace preferences in `kairo.json` take precedence
...
let Kairo detect an icon again.
...
Kairo only pulls when it can fast-forward and the checkout has no changed files, untracked files,
