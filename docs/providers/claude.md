...
This guide is for people who want to use more than one Claude setup in Kairo.
...
An empty `Claude HOME path` means Kairo uses your normal home directory.
...
Kairo only offers Claude providers that use the same Claude home for an existing thread. A
...
multiple files under its home directory, so Kairo keeps separate Claude homes isolated instead of
...
2. Add or configure a Claude provider in Kairo.
