...
- `vp run dev` — Starts contracts, server, and web through the development runner.
- `vp run dev:server` — Starts just the WebSocket server.
- `vp run dev:web` — Starts just the Vite dev server for the web app.
- Dev commands default `KAIRO_STATE_DIR` to `~/.kairo/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `vp run dev -- --base-dir ~/.kairo-2`
- `vp run start` — Runs the production server (serves built web app as static files).
- `vp run build` — Builds workspace applications and packages.
- `vp run typecheck` — Strict TypeScript checks for all packages.
- `vp test` — Runs workspace tests.
...
- Desktop production windows load the bundled UI from `kairo://app/index.html` (not a `127.0.0.1` document URL).
- Desktop packaging includes `apps/server/dist` (the `kairo` backend) and starts it on loopback with an auth token for WebSocket/API traffic.
...
Set `KAIRO_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `KAIRO_DEV_INSTANCE`)
- Example: `KAIRO_DEV_INSTANCE=branch-a vp run dev:desktop`

If you want full control instead of hashing, set `KAIRO_PORT_OFFSET` to a numeric offset.
