# Kairo Connect Clerk Setup

Kairo Connect uses one Clerk application for web, desktop, and mobile authentication. The relay accepts
Clerk JWTs only when they are generated from the `kairo-relay` template with the shared
`kairo-relay` audience.

## Application Keys

Kairo Connect is disabled in a fresh clone. To enable it for source builds, add a repository-root `.env`
or `.env.local` file:
...
absent, the `kairo connect` CLI command group is omitted. The bundled server still accepts runtime
...
forward that port before running `kairo connect login` or `kairo connect link`:
...
The app also adds a request-scoped `kairo_state` query parameter and validates it on callback. Initial
...
Approved signed-in users manage Kairo Connect under **Connections**. The web and desktop sidebars do
not expose a dedicated account or waitlist control. Signed-out users reach Clerk's waitlist and
sign-in flow contextually from the Kairo Connect controls on the Connections page.

On mobile, signed-out users open **Settings > Kairo Account** to reach `/settings/waitlist` within the
