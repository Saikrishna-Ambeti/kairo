...
Kairo has one runtime boundary: a client talks to a Kairo server over HTTP and WebSocket, and the server
...
One running Kairo server instance. It owns provider availability and auth, model availability, projects
...
| `RelayConnectionTarget`   | Managed Kairo Connect relay tunnels.                                     |
...
Tailscale is the first provider, and Kairo manages more than discovery. When `tailscaleServeEnabled` is
...
(`apps/server/src/server.ts`, using [`@kairo/tailscale`](../../packages/tailscale/src/tailscale.ts)).
...
Access answers one question: how does the client speak WebSocket to a Kairo server? It does not answer
how the server got started or who manages the process.

### Direct WebSocket access

`wss://kairo.example.com` or `ws://10.0.0.15:3773`, paired as a bearer target. This is the base model.
...
Managed Kairo Connect relay tunnels use `RelayConnectionTarget` and are the answer when the host is
...
Worker itself. See [kairo-connect.md](./kairo-connect.md).

### Tailscale access

A Kairo-managed `tailscale serve` mapping exposes the server on the tailnet over HTTPS, and the
...
[packages/ssh/src/tunnel.ts][sshtunnel], which resolves the target, launches or reuses the remote Kairo
...
Launch answers a different question: how does a Kairo server come to exist on the target machine? Keep
it separate from access.

- **Pre-existing server.** The operator already runs Kairo and the client connects directly or through a
  tunnel.
...
- **Client-managed local publish.** A local server is published through the relay with
  `kairo connect link`, exposing a desktop-hosted environment to mobile without router or firewall
  changes.
...
- a relay-hosted OAuth callback broker (see [kairo-connect.md](./kairo-connect.md));
