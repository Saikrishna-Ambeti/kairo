...
The launcher is the only runtime writer of `service-state.json`. `kairo service install` and
`kairo service update` may replace the launcher and state while the unit is stopped. Server children
...
1. The active server installs `kairo@<target>` into a unique staging directory.
...
```sh
npx kairo@<version> service update
```
