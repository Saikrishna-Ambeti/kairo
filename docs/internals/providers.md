...
A provider is the agent runtime that does the actual work. Kairo supports several, and the
...
Complete releases live in immutable version directories under the Kairo home
...
print it on stdout. Kairo accepts the exact native prefix on either stream and its browser-helper
...
scope. Only the initiating Kairo auth session receives its URL and flow ID or can complete or
...
event drain, then sends the replacement. Native background commands use Kairo's existing
...
`interaction_` requests are user questions, not approvals. Kairo keeps opaque option IDs in
...
`showInteractionModeToggle=false` keeps native `/plan` separate from Kairo Plan mode.
...
Chat adapters keep their own server per thread. They register a thread-specific `kairo` MCP
...
Kairo does not own an external OpenCode process. Native configuration changes there can require
an external reload or restart before Kairo's next refresh sees them.
