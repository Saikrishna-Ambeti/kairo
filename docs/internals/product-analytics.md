...
Kairo sends anonymous product events from the server to PostHog. The server
...
| `webDeployment`        | Web delivery: `hosted` for the hosted app or `server` for web files served by a Kairo server. Web only. This does not describe connection distance.                                             |
...
| `serverAppVersion` | Kairo server version.                                          |
...
- `platform`, `arch`, `wsl`, and `kairoCodeVersion` describe the server. Use the
