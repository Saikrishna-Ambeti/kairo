...
This document describes the target architecture for first-class remote environments in Kairo.
...
- Keep the Kairo server as the execution boundary.
...
Kairo already has a clean runtime boundary: the client talks to a Kairo server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one Kairo server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one Kairo server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the Kairo runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running Kairo server instance.
...
How does the client speak WebSocket to a Kairo server?
...
- best fit for hosted or self-managed remote Kairo deployments
...
For Kairo, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
Tailscale-backed access sits here architecturally even though the current implementation is endpoint discovery rather than a Kairo-managed tunnel. It contributes private-network endpoints and lets the existing HTTP/WebSocket client path do the actual connection.
...
- reach a machine
- probe it
- launch or reuse a remote Kairo server
...
How does a Kairo server come to exist on the target machine?
...
The user or operator already runs Kairo on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in Kairo:
...
The recommended Kairo flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies Kairo availability.
3. Desktop launches or reuses a remote Kairo server.
...
This is the inverse of remote launch: a local Kairo server is already running, and the client publishes it through a tunnel.
...
- A manually hosted Kairo server might be reached through direct `wss`.
...
Kairo already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
Kairo should not copy that part.

Kairo already has the right runtime boundary:

- one Kairo server per environment
- ordinary HTTP/WebSocket between client and environment

So Kairo should borrow Zed's launch discipline, not its transport protocol.
