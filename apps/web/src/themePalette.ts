...
import {
  ...
  type ThemeVariants,
} from "@kairo/shared/themePalettes";

export { EMBER_THEME, GROVE_THEME, IRIS_THEME, OCEAN_THEME, KAIRO_CHAT_THEME, THEME_COLOR_ROLES };
...
export const CUSTOM_THEMES_STORAGE_KEY = "kairo:themes:v1";
export const THEME_FOLLOW_SYSTEM_STORAGE_KEY = "kairo:theme-follow-system";
export const THEME_APPEARANCE_MODE_STORAGE_KEY = "kairo:theme-appearance-mode";
export const THEME_HALVES_STORAGE_KEY = "kairo:theme-halves:v1";

const LEGACY_KAIRO_CHAT_DARK_THEME_ID = "kairo-chat-dark";
...
const RESERVED_THEME_IDS = new Set([
  ...
  EMBER_THEME_ID,
  IRIS_THEME_ID,
  LEGACY_KAIRO_CHAT_DARK_THEME_ID,
  "kairo-grove",
  "kairo-ocean",
  "kairo-ember",
  "kairo-iris",
...
// Earlier builds shipped every maintainer theme under a kairo- prefix; only the
// genuinely Kairo-branded palette keeps it. Stored preferences and mixes with the
// old ids stay readable through this alias table.
const LEGACY_THEME_ID_ALIASES: Readonly<Record<string, string>> = {
  [LEGACY_KAIRO_CHAT_DARK_THEME_ID]: KAIRO_CHAT_THEME_ID,
  "kairo-grove": GROVE_THEME_ID,
  "kairo-ocean": OCEAN_THEME_ID,
  "kairo-ember": EMBER_THEME_ID,
  "kairo-iris": IRIS_THEME_ID,
};
...
export function canonicalThemePreference(theme: string): string {
  return theme === LEGACY_KAIRO_CHAT_DARK_THEME_ID ? theme : normalizeThemeId(theme);
}
...
// Older builds stored the dark Kairo Chat palette as a separate theme. Keep
...
function legacyThemeMode(theme: ThemePreference): ThemeAppearance | null {
  return theme === LEGACY_KAIRO_CHAT_DARK_THEME_ID ? "dark" : null;
}

/**
 * The palette Kairo wears with no theme installed, captured from the app's
 ...
 */
const KAIRO_CODE_LIGHT_THEME_COLORS: ThemeColors = {
  canvas: "#fcfcfc",
  chrome: "#fcfcfc",
  toolbar: "#fcfcfc",
...
const KAIRO_CODE_DARK_THEME_COLORS: ThemeColors = {
  canvas: "#0a0a0a",
  chrome: "#0a0a0a",
  toolbar: "#0a0a0a",
...
/**
 * The standard Kairo look as a theme palette, for seeding a new theme when
 * no theme is installed. Distinct from {@link getDefaultThemeColors}, which
 * carries the flagship Kairo Chat palette used to fill roles omitted by theme
 * files.
 */
export function getStandardThemeColors(appearance: ThemeAppearance): ThemeColors {
  if (appearance === "dark") {
    return (standardDarkThemeColors ??= decodeThemeColors(KAIRO_CODE_DARK_THEME_COLORS));
  }
  return (standardLightThemeColors ??= decodeThemeColors(KAIRO_CODE_LIGHT_THEME_COLORS));
}
...
/**
 * The status colors Kairo shows without a theme, read from the app's own
...
export function getDefaultThemeColors(appearance: ThemeAppearance): ThemeColors {
  return appearance === "dark" ? KAIRO_CHAT_THEME.variants!.dark! : KAIRO_CHAT_THEME.colors;
}
