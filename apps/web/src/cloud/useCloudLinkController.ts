...
/**
 * Drives the primary environment's Kairo Connect link. Kairo Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    ...
    console.error("[kairo-connect] Could not update Kairo Connect", { message, traceId, cause });
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Kairo Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Kairo Connect before enabling this."));
        return false;
      }
