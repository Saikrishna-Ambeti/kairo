...
import { EnvironmentRegistry } from "@kairo/client-runtime/connection";
import { request, runStream } from "@kairo/client-runtime/rpc";
import { makeEnvironmentHttpApiClient } from "@kairo/client-runtime/rpc";
import { ManagedRelay, relayProtectedErrorMessage } from "@kairo/client-runtime/relay";
  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Kairo cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Kairo cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
> {
  return Effect.gen(function* () {
    ...
    if (!configuredRelayUrl) {
      return yield* new CloudEnvironmentLinkError({
        message: "KAIRO_RELAY_URL is not configured.",
      });
    }
...
// independently of Kairo Connect while clients reach the environment out of band.
