...
describe("theme files", () => {
  ...
  it("derives a readable palette from extreme simple-editor colors", () => {
    ...
    // Status colors fall back to Kairo's standard red and amber rather than
  ...
  it("canonicalizes the explicitly exported theme", () => {
    const serialized = serializeThemeFile({
      ...KAIRO_CHAT_THEME,
      colors: { ...KAIRO_CHAT_THEME.colors, accent: "hsl(263 70% 58%)" },
    });
    expect(JSON.parse(serialized)).toMatchObject({
      version: THEME_FILE_VERSION,
      id: KAIRO_CHAT_THEME.id,
      name: KAIRO_CHAT_THEME.label,
      appearance: "light",
  ...
  it("suppresses sidebar artwork during a live custom-theme preview", () => {
    ...
    applyThemeColorPreview(KAIRO_CHAT_THEME.colors, "light");
  ...
  it("keeps optional light and dark palettes under one theme id", () => {
    ...
    expect(getThemeModes(KAIRO_CHAT_THEME)).toEqual(["light", "dark"]);
    expect(resolveThemeAppearance(KAIRO_CHAT_THEME.id, true, true)).toBe("dark");
    expect(resolveDesktopTheme(KAIRO_CHAT_THEME.id, true)).toBe("system");
    expect(resolveThemeAppearance(KAIRO_CHAT_THEME.id, false, false, "dark")).toBe("dark");
    expect(resolveDesktopTheme(KAIRO_CHAT_THEME.id, false, "dark")).toBe("dark");
  ...
  it("keeps the Kairo Chat palette faithful and readable", () => {
    expectThemeColors(KAIRO_CHAT_THEME.colors, {
      canvas: "#fdf7fd",
      chrome: "#fdf7fd",
    ...
    expectThemeColors(KAIRO_CHAT_THEME.variants!.dark!, {
      canvas: "#1f1a24",
      chrome: "#1f1a24",
      surface: "#29232d",
    ...
    for (const mode of ["light", "dark"] as const) {
      const colors = getThemeColorsForMode(KAIRO_CHAT_THEME, mode)!;
      expect(contrastRatio(colors.text, colors.canvas)).toBeGreaterThanOrEqual(7);
      expect(contrastRatio(colors.textMuted, colors.canvas)).toBeGreaterThanOrEqual(4.5);
      expect(contrastRatio(colors.messageForeground, colors.messageSurface)).toBeGreaterThanOrEqual(
        4.5,
  ...
  it("includes the dual-mode maintainer themes", () => {
    for (const theme of [KAIRO_CHAT_THEME, GROVE_THEME, OCEAN_THEME, EMBER_THEME, IRIS_THEME]) {
...
describe("stored theme preferences", () => {
  ...
  it("resolves the legacy kairo-chat-dark preference to dark Kairo Chat", () => {
    expect(getThemeDefinition("kairo-chat-dark")).toBe(KAIRO_CHAT_THEME);
    expect(getThemePreferenceMode("kairo-chat-dark")).toBe("dark");
    expect(resolveThemeAppearance("kairo-chat-dark", true, false)).toBe("dark");
    expect(resolveDesktopTheme("kairo-chat-dark", false)).toBe("dark");
    expect(isKnownThemePreference("kairo-chat-dark")).toBe(true);
  });

  it("resolves legacy kairo-prefixed ids onto the renamed themes", () => {
    for (const [legacy, theme] of [
      ["kairo-grove", GROVE_THEME],
      ["kairo-ocean", OCEAN_THEME],
      ["kairo-ember", EMBER_THEME],
      ["kairo-iris", IRIS_THEME],
    ] as const) {
    ...
    expect(canonicalThemePreference("kairo-chat-dark")).toBe("kairo-chat-dark");
    // A stored mix that predates the rename resolves to the new ids.
    expect(parseThemeHalves(JSON.stringify({ light: "kairo-ocean", dark: "kairo-grove" }))).toEqual(
      {
  ...
  it("recognizes only preferences the runtime can render", () => {
    for (const preference of ["light", "dark", "system", KAIRO_CHAT_THEME.id, GROVE_THEME.id]) {
...
describe("singleAppearanceOf", () => {
  it("reports the only half a theme can claim, and null for a pair", () => {
    const { variants: _pair, ...base } = KAIRO_CHAT_THEME;
    expect(singleAppearanceOf({ ...base, id: "x", appearance: "dark" })).toBe("dark");
    expect(singleAppearanceOf(KAIRO_CHAT_THEME)).toBe(null);
  });
});
