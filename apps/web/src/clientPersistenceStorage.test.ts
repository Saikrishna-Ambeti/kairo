import { DEFAULT_CLIENT_SETTINGS } from "@kairo/contracts";
...
describe("clientPersistenceStorage", () => {
  ...
  it.each(["not-json", '{"wordWrap":"invalid"}'])(
    ...
    async (value) => {
      ...
      testWindow.localStorage.setItem("kairo:client-settings:v1", value);
      ...
      expect(() => readBrowserClientSettings()).toThrow(
        expect.objectContaining({
          ...
          operation: "decode",
          storageKey: "kairo:client-settings:v1",
        }),
      );
      expect(testWindow.localStorage.getItem("kairo:client-settings:v1")).toBe(value);
    },
  );
  ...
  it("preserves saved settings across a transient read failure", async () => {
    ...
    testWindow.localStorage.setItem("kairo:client-settings:v1", JSON.stringify(settings));
  ...
  it("keeps the diff layout across reloads and defaults it to stacked", async () => {
    ...
    testWindow.localStorage.setItem("kairo:client-settings:v1", JSON.stringify({}));
