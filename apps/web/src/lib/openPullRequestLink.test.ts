...
import { ProjectId, type RepositoryIdentity } from "@kairo/contracts";
...
describe("pullRequestCandidateUrlFromReferenceAutolink", () => {
  it("turns GitHub's shared issue route into a pull request candidate", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/pingdotgg/kairo/issues/8600#issuecomment-1",
      ),
    ).toBe("https://github.com/pingdotgg/kairo/pull/8600#issuecomment-1");
  });

  it("does not reinterpret other issue hosts or malformed references", () => {
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://gitlab.com/pingdotgg/kairo/-/issues/8600",
      ),
    ).toBeNull();
    expect(
      pullRequestCandidateUrlFromReferenceAutolink(
        "https://github.com/pingdotgg/kairo/issues/not-a-number",
      ),
    ).toBeNull();
  });
});
...
describe("matchesLinkedPullRequestUrl", () => {
  ...
  it("matches the same pull request without looking up its project", () => {
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.com/PingDotGG/Kairo/pull/42/files",
      ),
    ).toBe(true);
  });
  ...
  it("rejects a different pull request or host", () => {
    expect(
      matchesLinkedPullRequestUrl(linkedPullRequest, "https://github.com/pingdotgg/kairo/pull/43"),
    ).toBe(false);
    expect(
      matchesLinkedPullRequestUrl(
        linkedPullRequest,
        "https://github.example.com/pingdotgg/kairo/pull/42",
      ),
    ).toBe(false);
  });
});
...
describe("parseChangeRequestUrl", () => {
  it("reads a GitHub pull request", () => {
    expect(parseChangeRequestUrl("https://github.com/Kairo/Kairo/pull/123")).toEqual({
      host: "github.com",
      repository: "kairo/kairo",
      number: 123,
    });
  });
  ...
  it("reads a GitLab merge request, nested groups and all", () => {
    expect(
      parseChangeRequestUrl("https://gitlab.com/kairo/platform/kairo/-/merge_requests/42"),
    ).toEqual({
      host: "gitlab.com",
      repository: "kairo/platform/kairo",
      number: 42,
    });
  });
  ...
  it("reads both Azure DevOps URL forms, keeping `_git` in the repository path", () => {
    expect(
      parseChangeRequestUrl("https://dev.azure.com/acme/platform/_git/kairo/pullrequest/17"),
    ).toEqual({
      host: "dev.azure.com",
      repository: "acme/platform/_git/kairo",
      number: 17,
    });
    expect(
      parseChangeRequestUrl("https://acme.visualstudio.com/platform/_git/kairo/pullrequest/17"),
    ).toEqual({
      host: "acme.visualstudio.com",
      repository: "platform/_git/kairo",
      number: 17,
    });
  });

  it("survives trailing segments, a trailing slash and a query string", () => {
    expect(parseChangeRequestUrl("https://github.com/kairo/kairo/pull/123/files?w=1")).toEqual({
      host: "github.com",
      repository: "kairo/kairo",
      number: 123,
    });
  ...
  it("claims nothing it cannot be sure of, so the link goes to the browser", () => {
    for (const link of [
      "https://github.com/kairo/kairo/issues/123",
      "https://github.com/kairo/kairo/commit/0a1b2c3",
      "https://github.com/kairo/kairo",
      "https://github.com/kairo/kairo/pull/abc",
      "https://gitlab.com/kairo/kairo/-/snippets/12",
      "https://gitlab.com/kairo/kairo/-/issues/12",
      ...
      "javascript:alert(1)//github.com/kairo/kairo/pull/1",
      "not a url",
    ]) {
...
describe("findProjectForChangeRequest", () => {
  ...
  it("matches a nested GitLab group by the whole path below the host", () => {
    ...
    // two-segment owner/name form would look for `kairo/kairo` and find nothing.
    const projects = [
      project({
        canonicalKey: "gitlab.com/kairo/platform/kairo",
        provider: "gitlab",
        displayName: "kairo/platform/kairo",
        owner: "kairo",
        name: "kairo",
      }),
    ];
  ...
  it("keeps two hosts apart, so an Enterprise link does not open the public one", () => {
    const projects = [
      project({
        canonicalKey: "github.com/pingdotgg/kairo",
        provider: "github",
        owner: "pingdotgg",
        name: "kairo",
      }),
