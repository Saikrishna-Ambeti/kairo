...
describe("index.html boot script", () => {
  const parityCases: ReadonlyArray<{
    ...
    {
      name: "Kairo Chat follows a dark OS",
      storage: { [THEME_STORAGE_KEY]: "kairo-chat", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "an explicit global dark mode applies to Kairo Chat",
      storage: {
        [THEME_STORAGE_KEY]: "kairo-chat",
    ...
    {
      name: "a legacy kairo-grove preference resolves through the alias",
      storage: { [THEME_STORAGE_KEY]: "kairo-grove", [THEME_FOLLOW_SYSTEM_STORAGE_KEY]: "true" },
      prefersDark: true,
    },
    {
      name: "legacy kairo-chat-dark resolves to dark Kairo Chat",
      storage: { [THEME_STORAGE_KEY]: "kairo-chat-dark" },
      prefersDark: true,
    },
    {
  ...
  it("marks built-in and custom themes on the document element", () => {
    ...
    expect(chat.themeId).toBe("kairo-chat");
  ...
  it("applies the matching half of an automatic mix to the splash", () => {
    const storage = {
      ...
      "kairo:theme-halves:v1": JSON.stringify({ dark: GROVE_THEME.id }),
    };
    ...
    expect(light.themeId).toBe("kairo-chat");
    expect(light.bootVariables["--boot-background"]).toBe(
      getThemeColorsForMode(KAIRO_CHAT_THEME, "light")!.canvas,
    );
  });
  ...
  it("resolves a legacy-prefixed mix half onto the renamed theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "kairo-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "kairo:theme-halves:v1": JSON.stringify({ dark: "kairo-grove" }),
      },
      prefersDark: true,
    });
    expect(boot.isDark).toBe(true);
  ...
  it("ignores a mix half that names an unknown theme", () => {
    const boot = runBootScript({
      storage: {
        [THEME_STORAGE_KEY]: "kairo-chat",
        [THEME_APPEARANCE_MODE_STORAGE_KEY]: "system",
        "kairo:theme-halves:v1": JSON.stringify({ dark: "gone-theme" }),
      },
      prefersDark: true,
    });
    expect(boot.themeId).toBe("kairo-chat");
    expect(boot.isDark).toBe(true);
  });
