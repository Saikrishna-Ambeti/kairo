import type { EnvironmentId, ServerConfig, ServerSelfUpdateCapability } from "@kairo/contracts";
import type { ServerUpdateState } from "@kairo/client-runtime/state/server";
import { compareSemverVersions, parseSemver } from "@kairo/shared/semver";
...
export const VERSION_MISMATCH_DISMISSALS_STORAGE_KEY = "kairo:version-mismatch-dismissals:v1";
...
/**
 * The skew a user can act on: the connected server runs an older Kairo than
...
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Try syncing the client and server to the same Kairo version.",
  };
}
...
export function manualServerUpdateCommand(targetVersion: string): string {
  return `npx kairo@${targetVersion}`;
}
