...
import type { UsageLimitSourceSnapshots } from "@kairo/contracts";
import {
...
import { derivePendingRequests } from "@kairo/client-runtime/pending-requests";
import {
...
import { wasBootstrapThreadDeleted } from "@kairo/client-runtime/errors";
import { type CodexArtifactTemplate } from "@kairo/client-runtime/codex-artifact-templates";
import { effectiveSnoozed, threadWokeAt } from "@kairo/client-runtime/state/thread-settled";
import {
...
import {
  ...
  scopeProjectRef,
  scopeThreadRef,
} from "@kairo/client-runtime/environment";
import {
...
} from "@kairo/shared/projectScripts";
...
} from "@kairo/shared/terminalLabels";
...
import { assistantCitationsToPlainText } from "@kairo/shared/assistantCitations";
...
} from "@kairo/client-runtime/state/subagentRuntime";
...
import { resolveProviderSkillsForCwd } from "@kairo/client-runtime/providerSkills";
...
export default function ChatView(props: ChatViewProps) {
  ...
  // Kairo owns /usage-limits only where Limits has data for the selected provider;
  ...
  const [resumeCompactionPermanentlyDismissed, setResumeCompactionPermanentlyDismissed] =
    useLocalStorage(
      `kairo:resume-compaction-dismissed:${environmentId}:${activeProviderInstanceId ?? "claudeAgent"}`,
      false,
      Schema.Boolean,
    );
  ...
  // the sent message can stay near the top. Kairo only owns streaming adjustments
  ...
  const onRespondToUserInput = useCallback(
    async (requestId: ApprovalRequestId, answers: Record<string, unknown>) => {
      ...
      const attachmentsByQuestionId = new Map<
        string,
        import("@kairo/contracts").UserInputAttachments[string]
      >();
      for (const question of activePendingUserInput.questions) {
        ...
        attachmentsByQuestionId.set(
          question.id,
          uploaded as import("@kairo/contracts").UserInputAttachments[string],
        );
      }
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
      ...
      <PullRequestsUnavailableState
        ...
        error="Update this environment's Kairo server to browse pull requests."
      />
