...
describe("openPreviewSession", () => {
  ...
  it("applies the RPC response without waiting for a preview event", async () => {
    ...
    expect(readThreadPreviewState(threadRef).recentlySeenUrls).toEqual(["https://kairo.chat/"]);
  });
  ...
  it.each(["session", "link"] as const)(
    ...
    async (entryPoint) => {
      ...
      const input = { openPreview, threadRef, url: "https://kairo.chat/" };
