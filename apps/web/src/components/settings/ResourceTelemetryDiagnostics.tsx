...
export function ResourceTelemetryDiagnostics() {
  ...
  const allKairo = snapshot?.groups.allKairo;
        ...
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03),0_8px_30px_rgb(0_0_0/0.035)]">
          ...
          <div className="grid grid-cols-2 divide-x divide-y divide-border/55 md:grid-cols-3">
            <IconStat
              ...
              label="Current CPU"
              value={allKairo ? `${allKairo.currentCpuPercent.toFixed(1)}%` : "..."}
              detail={
                allKairo ? `${formatCpuTime(allKairo.cpuTimeMs)} observed CPU time` : undefined
              }
            />
            <IconStat
              icon={<MemoryStickIcon className="size-3.5" />}
              label="Resident memory"
              value={allKairo ? formatBytes(allKairo.currentRssBytes) : "..."}
              detail={
                allKairo
                  ? `${formatBytes(allKairo.peakRssBytes)} combined process peaks`
                  : undefined
              }
            />
            <IconStat
              ...
              label="Process count"
              value={allKairo ? String(allKairo.processCount) : "..."}
              detail={
                allKairo
                  ? `${allKairo.processStarts} starts · ${allKairo.processExits} exits`
                  : undefined
              }
            />
            <IconStat
              ...
              value={allKairo ? formatRate(allKairo.ioReadBytesPerSecond) : "..."}
              detail={allKairo ? `${formatBytes(allKairo.ioReadBytes)} observed` : undefined}
            />
            <IconStat
              icon={<DatabaseIcon className="size-3.5" />}
              label="Write throughput"
              value={allKairo ? formatRate(allKairo.ioWriteBytesPerSecond) : "..."}
              detail={allKairo ? `${formatBytes(allKairo.ioWriteBytes)} observed` : undefined}
              tone={
                allKairo && allKairo.ioWriteBytesPerSecond >= 10 * 1_024 * 1_024
                  ? "danger"
                  : allKairo && allKairo.ioWriteBytesPerSecond >= 1_024 * 1_024
                    ? "warning"
                    : "default"
              }
            />
      ...
      >
        <div className="overflow-hidden rounded-2xl border border-border/70 bg-card shadow-[0_1px_1px_rgb(0_0_0/0.03)]">
          <div className="bg-muted/15 px-4 py-3 text-[11px] leading-relaxed text-muted-foreground sm:px-5">
            ...
            counters identify known Kairo operations so process spikes can be correlated with
