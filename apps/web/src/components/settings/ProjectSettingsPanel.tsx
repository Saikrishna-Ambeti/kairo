...
import { scopeProjectRef } from "@kairo/client-runtime/environment";
...
import type {
  ...
  KairoProjectFileScript,
  ThreadEnvMode,
} from "@kairo/contracts";
import { resolveEnvModeLabel } from "../BranchToolbar.logic";
import { createModelSelection } from "@kairo/shared/model";
import { DEFAULT_RESOLVED_KEYBINDINGS } from "@kairo/shared/keybindings";
...
import { useKairoProjectFileState } from "../../hooks/useKairoProjectFileScripts";
...
function ProjectDetail({ group }: { group: SidebarProjectSnapshot }) {
  ...
  const kairoFile = useKairoProjectFileState(
  ...
  // repo's kairo.json value when present, otherwise the global setting.
  const inheritedEnvMode = kairoFile.file?.defaultThreadEnvMode ?? settings.defaultThreadEnvMode;
  const inheritedEnvModeSource =
    kairoFile.file?.defaultThreadEnvMode != null ? "kairo.json" : "global";
  const importableScripts = useMemo(
    () =>
      kairoFile.scripts.filter(
        (fileScript) =>
          !scripts.some(
            (script) =>
  ...
  const importFileScript = useCallback(
    async (fileScript: KairoProjectFileScript) => {
    ...
    <>
      <SettingsPageContainer>
        ...
        <SettingsSection title="New threads">
          ...
          <SettingsRow
            title="Workspace"
            description="Where new threads in this project start. Overrides kairo.json and the global default; applies to every checkout in this group."
            resetAction={
              storedEnvMode !== null ? (
                <SettingResetButton
              ...
              >
                ...
                <SelectPopup align="end" alignItemWithTrigger={false}>
                  <SelectItem value="inherit">
                    {group.memberProjects.length > 1
                      ? "Default (each checkout's kairo.json or global setting)"
        ...
        >
              ...
              {importableScripts.length > 0 ? (
                <Menu>
                  ...
                  <MenuPopup align="end" className="w-72">
                    <MenuGroup>
                      <MenuGroupLabel>Import from kairo.json</MenuGroupLabel>
          ...
          {kairoFile.status === "invalid" ? (
            <SettingsRow
              title="kairo.json is invalid"
              description="A kairo.json exists in this checkout but fails to parse, so every action and icon it declares is ignored. Check the JSON syntax and icon values."
