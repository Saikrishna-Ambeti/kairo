...
import { WsRpcClient } from "@kairo/client-runtime";
import type { RelayClientEnvironmentRecord } from "@kairo/contracts/relay";
...
import { useKairoConnectAuthPrompt } from "../clerk/useKairoConnectAuthPrompt";
...
function CloudLinkSwitch({
  ...
  const control = (
    <Switch
      aria-label="Enable Kairo Connect"
      checked={checked}
      disabled={disabled}
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const updateLink = async (enabled: boolean) => {
    ...
    try {
      ...
      if (enabled) {
        if (!clerkToken) {
          throw new Error("Sign in to Kairo Connect before linking this environment.");
        }
      ...
      toastManager.add({
        type: "success",
        title: enabled ? "Kairo Connect linked" : "Kairo Connect unlinked",
        description: enabled
      ...
      const message =
        cause instanceof Error ? cause.message : "Could not update Kairo Connect access.";
      setOperationError(message);
      toastManager.add({
        type: "error",
        title: "Could not update Kairo Connect",
        description: message,
      });
    } finally {
      setIsUpdating(false);
  ...
  const updatePublishAgentActivity = async (enabled: boolean) => {
    ...
    try {
      ...
      toastManager.add({
        type: "error",
        title: "Could not update Kairo Connect preferences",
        description:
...
function ConfiguredCloudRemoteEnvironmentRows({
  ...
  const connectEnvironment = async (environment: RelayClientEnvironmentRecord) => {
    ...
    try {
      const clerkToken = await getToken(resolveRelayClerkTokenOptions());
      if (!clerkToken) {
        throw new Error("Sign in to Kairo Connect before connecting this environment.");
      }
      ...
      toastManager.add({
        ...
        description: `${connection.label} is available through Kairo Connect.`,
      });
    } catch (cause) {
      toastManager.add({
        type: "error",
        ...
        description:
          cause instanceof Error
            ? cause.message
            : "Could not connect the Kairo Connect environment.",
      });
    } finally {
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <ConnectionStatusDot
              ...
              tooltipText="Available through Kairo Connect"
            />
            <p className="truncate text-sm font-medium">{environment.label}</p>
          </div>
          <p className="mt-1 truncate text-xs text-muted-foreground">Kairo Connect</p>
        </div>
        <Button
          size="sm"
