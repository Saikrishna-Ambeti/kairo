...
export function ComposioSettingsPanel() {
  ...
  const refresh = useCallback(async () => {
    const next = await serverApi.getComposioStatus();
    setStatus((previous) => {
    ...
    try {
      const next =
        mode === "install_and_login"
          ? await serverApi.installAndLoginComposio(
              { providerInstanceIds: selectedProviderIds },
              appendProgress,
            )
          : await serverApi.loginComposio(
              { providerInstanceIds: selectedProviderIds },
              appendProgress,
            );
      setStatus(next);
  ...
  const connectToolkit = async (toolkit: string) => {
    ...
    setBusy(true);
    try {
      const next = await serverApi.linkComposioToolkit({ toolkit }, appendProgress);
      setStatus(next);
  ...
  const toggleProvider = async (providerInstanceId: ProviderInstanceId, checked: boolean) => {
    ...
    try {
      const next = await serverApi.installComposioAgentSupport({
