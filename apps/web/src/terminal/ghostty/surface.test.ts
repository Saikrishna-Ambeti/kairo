...
describe("terminalLinkAtColumn", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    const cells = [
      cell("🙂"),
      cell(""),
      ...Array.from("https://kairo.codes", (character) => cell(character)),
    ];
    ...
    expect(terminalLinkAtColumn(row, 2)).toBe("https://kairo.codes");
    expect(terminalLinkAtColumn(row, cells.length - 1)).toBe("https://kairo.codes");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    const tailCut = [row("https://kairo.codes", false, true)];
    expect(terminalLinkAtPosition(tailCut, 0, 8)).toBeNull();
    // A partial bottom row is provably complete and still resolves.
    const complete = [row("https://kairo.codes", false), row("", false)];
    expect(terminalLinkAtPosition(complete, 0, 8)).toBe("https://kairo.codes");
    ...
    const wideFull: GhosttyRow = {
      cells: [
        { ...cell("🙂"), wide: 1 },
        { ...cell(""), wide: 2 },
        ...Array.from("https://kairo.code", (character) => cell(character)),
      ],
      text: "🙂 https://kairo.code",
      isWrapContinuation: false,
      wrapsToNext: true,
    };
    ...
    const unwrittenTail: GhosttyRow = {
      ...
      text: "https://kairo.codes",
      isWrapContinuation: false,
      wrapsToNext: false,
    };
    expect(terminalLinkAtPosition([unwrittenTail], 0, 8)).toBe("https://kairo.codes");
  });
});
