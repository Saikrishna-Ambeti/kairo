...
describe("terminalLinkAtPositionWithRange", () => {
  it("maps terminal cells to UTF-16 offsets after a wide emoji", () => {
    ...
    expect(terminalLinkAtPositionWithRange([row], 0, 2)?.text).toBe("https://kairo.codes");
  ...
  it("refuses links truncated at the viewport edges instead of mis-resolving", () => {
    ...
    expect(terminalLinkAtPositionWithRange(complete, 0, 8)?.text).toBe("https://kairo.codes");
    ...
    expect(terminalLinkAtPositionWithRange([unwrittenTail], 0, 8)?.text).toBe("https://kairo.codes");
  });
});
