...
fork, with Kairo's iOS 16 compatibility patch applied. `KairoTerminalView` owns a `libghostty` surface and
...
Android currently implements the same view name (`KairoTerminalSurface`) and event payloads so the
