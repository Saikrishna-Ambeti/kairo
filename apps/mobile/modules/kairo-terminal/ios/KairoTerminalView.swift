...
public final class KairoTerminalView: ExpoView, UITextFieldDelegate {
  ...
  var terminalKey: String = "" {
    didSet {
      accessibilityIdentifier = "kairo-terminal-\(terminalKey)"
  ...
  private func setupWriteCallback() {
    ...
    ghostty_surface_set_write_callback(surface, { userdata, data, len in
      ...
      let view = Unmanaged<KairoTerminalView>.fromOpaque(userdata).takeUnretainedValue()
  ...
  private func resizeSurface() {
    ...
    let width = UInkairo2(max(floor(terminalViewport.bounds.width * scale), 1))
    let height = UInkairo2(max(floor(terminalViewport.bounds.height * scale), 1))
  ...
  private func writeThemeConfigFile() -> String? {
    ...
    let url = URL(fileURLWithPath: NSTemporaryDirectory())
      .appendingPathComponent("kairo-terminal-theme-\(appearance.rawValue).ghostty")
