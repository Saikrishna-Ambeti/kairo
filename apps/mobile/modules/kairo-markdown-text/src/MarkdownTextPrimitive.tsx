...
import KairoMarkdownTextRunNativeComponent from "./KairoMarkdownTextRunNativeComponent";
import KairoMarkdownTextNativeComponent from "./KairoMarkdownTextNativeComponent";
...
function MarkdownTextPrimitiveChild({ style, children, ...rest }: MarkdownTextPrimitiveProps) {
  ...
  const nativeChildren = React.Children.toArray(children).map((child) => {
    ...
    return (
      ...
      <KairoMarkdownTextRunNativeComponent
  ...
  if (!isAncestor) {
    return (
      <TextAncestorContext.Provider value={contextValue}>
        <KairoMarkdownTextNativeComponent
          {...textDefaults}
          {...rest}
