...
import { installMarkdownCopySanitizer } from "./KairoMarkdownTextSelectionModule";
...
export function NativeMarkdownSelectableText(props: {
  ...
  // KairoMarkdownText only rebuilds its attributed string during native layout. A
        ...
        return (
          <MarkdownTextPrimitive
            ...
            nativeID={
              Platform.OS === "ios"
                ? run.fileIcon
                  ? `kairo-file:${Image.resolveAssetSource(markdownFileIconSource(run.fileIcon)).uri}`
                  : run.skillName
                    ? "kairo-skill:sf:cube"
                    : linkIcon
                      ? `kairo-link:${Image.resolveAssetSource(markdownLinkIconSource(linkIcon)).uri}`
                      : undefined
                : undefined
            }
