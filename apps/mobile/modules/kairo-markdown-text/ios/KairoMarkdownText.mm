...
static void KairoMarkdownTextApplyAttachments(
    NSMutableAttributedString *attributedString,
...
@protocol KairoMarkdownOutsideTapTarget <NSObject>
- (void)clearSelectionForOutsideTapWithHitView:(UIView *)hitView;
@end

@interface KairoMarkdownOutsideTapCoordinator : NSObject <UIGestureRecognizerDelegate>

- (instancetype)initWithWindow:(UIWindow *)window;
- (void)addTarget:(id<KairoMarkdownOutsideTapTarget>)target;
- (void)removeTarget:(id<KairoMarkdownOutsideTapTarget>)target;

@end

static const void *KairoMarkdownOutsideTapCoordinatorKey =
    &KairoMarkdownOutsideTapCoordinatorKey;

@implementation KairoMarkdownOutsideTapCoordinator {
  __weak UIWindow *_window;
  UITapGestureRecognizer *_recognizer;
  NSHashTable<id<KairoMarkdownOutsideTapTarget>> *_targets;
}
...
- (void)removeTarget:(id<KairoMarkdownOutsideTapTarget>)target
{
  ...
  if (objc_getAssociatedObject(window, KairoMarkdownOutsideTapCoordinatorKey) == self) {
...
- (void)handleTap:(UITapGestureRecognizer *)sender
{
  ...
  for (id<KairoMarkdownOutsideTapTarget> target in _targets.allObjects) {
...
static KairoMarkdownOutsideTapCoordinator *
KairoMarkdownOutsideTapCoordinatorForWindow(UIWindow *window)
{
  KairoMarkdownOutsideTapCoordinator *coordinator =
      objc_getAssociatedObject(window, KairoMarkdownOutsideTapCoordinatorKey);
  if (coordinator == nil) {
    coordinator = [[KairoMarkdownOutsideTapCoordinator alloc] initWithWindow:window];
    objc_setAssociatedObject(
        window,
        KairoMarkdownOutsideTapCoordinatorKey,
        coordinator,
...
@interface KairoMarkdownText () <RCTKairoMarkdownTextViewProtocol, UIGestureRecognizerDelegate, UITextViewDelegate>

@end

@interface KairoMarkdownText () <KairoMarkdownOutsideTapTarget>
@end
...
{
  return concreteComponentDescriptorProvider<KairoMarkdownTextComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const KairoMarkdownTextProps>();
    _props = defaultProps;
...
- (void)didMoveToWindow
{
  ...
  if (_outsideTapWindow != nil) {
    KairoMarkdownOutsideTapCoordinator *coordinator =
        objc_getAssociatedObject(_outsideTapWindow, KairoMarkdownOutsideTapCoordinatorKey);
    [coordinator removeTarget:self];
  }
  _outsideTapWindow = self.window;
  if (_outsideTapWindow != nil) {
    [KairoMarkdownOutsideTapCoordinatorForWindow(_outsideTapWindow) addTarget:self];
  }
}
...
- (void)drawRect:(CGRect)rect
{
  ...
  const auto &props = *std::static_pointer_cast<KairoMarkdownTextProps const>(_props);
  ...
  KairoMarkdownTextApplyParagraphStyles(
      convertedAttrString,
      _state->getData().paragraphStyleRanges);
  KairoMarkdownTextApplyAttachments(
      convertedAttrString,
  ...
  for (UIView *child in self.subviews) {
    ...
    KairoMarkdownTextRun *textChild = (KairoMarkdownTextRun *)child;
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::KairoMarkdownTextEventEmitter>(_eventEmitter)
    ->onTextLayout(facebook::react::KairoMarkdownTextEventEmitter::OnTextLayout{static_cast<int>(self.tag), lines});
  };
}

- (void)loadAttachmentImages:(const std::vector<KairoMarkdownTextAttachmentRange> &)attachmentRanges
{
  for (const auto &attachmentRange : attachmentRanges) {
    NSString *imageUri = [NSString stringWithUTF8String:attachmentRange.imageUri.c_str()];
    if ([imageUri hasPrefix:@"sf:"]) {
...
- (void)updateProps:(Props::Shared const &)props oldProps:(Props::Shared const &)oldProps
{
  ...
  if (oldViewProps.ellipsizeMode != newViewProps.ellipsizeMode) {
    if (newViewProps.ellipsizeMode == KairoMarkdownTextEllipsizeMode::Head) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingHead;
    } else if (newViewProps.ellipsizeMode == KairoMarkdownTextEllipsizeMode::Middle) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingMiddle;
    } else if (newViewProps.ellipsizeMode == KairoMarkdownTextEllipsizeMode::Tail) {
      _textView.textContainer.lineBreakMode = NSLineBreakMode::NSLineBreakByTruncatingTail;
    } else if (newViewProps.ellipsizeMode == KairoMarkdownTextEllipsizeMode::Clip) {
...
{
  _state = std::static_pointer_cast<const KairoMarkdownTextShadowNode::ConcreteState>(state);
  [self setNeedsDisplay];
}
...
- (nullable KairoMarkdownTextRun *)childForCharacterRange:(NSRange)characterRange
{
...
- (KairoMarkdownTextRun*)getTouchChild:(CGPoint)location
{
               ...
               defaultAction:(UIAction *)defaultAction API_AVAILABLE(ios(17.0))
{
  ...
  __weak KairoMarkdownTextRun *weakChild = child;
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
  ...
  std::dynamic_pointer_cast<const facebook::react::KairoMarkdownTextEventEmitter>(_eventEmitter)
    ->onSelectionChange(facebook::react::KairoMarkdownTextEventEmitter::OnSelectionChange{
...
Class<RCTComponentViewProtocol> KairoMarkdownTextCls(void)
{
  return KairoMarkdownText.class;
}
