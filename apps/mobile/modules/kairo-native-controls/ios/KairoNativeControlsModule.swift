...
public final class KairoNativeControlsModule: Module {
  private let presentationSources = KairoPresentationSources()
  private var videoPresentation: KairoNativeVideoPresentation?
  private var filePresentation: KairoNativeFilePresentation?

  public func definition() -> ModuleDefinition {
    Name("KairoNativeControls")
    ...
    View(KairoPresentationSourceView.self) {
      ViewName("PresentationSource")
      Prop("identifier") { (view: KairoPresentationSourceView, identifier: String) in
    ...
    Function("getShowcaseScene") { () -> String? in
      let scenePath = NSHomeDirectory() + "/Library/Caches/KairoShowcaseScene"
    ...
    AsyncFunction("applyShowcaseOrientation") { (orientation: String) in
      ...
      for case let windowScene as UIWindowScene in UIApplication.shared.connectedScenes {
        windowScene.requestGeometryUpdate(.iOS(interfaceOrientations: mask)) { error in
          NSLog("KairoNativeControls applyShowcaseOrientation(\(orientation)) failed: \(error)")
        }
    ...
    Function("markShowcaseReady") { (scene: String) in
      let readyPath = NSHomeDirectory() + "/Library/Caches/KairoShowcaseReadyScene"
  ...
  private func presentVideo(url: URL, title: String, sourceIdentifier: String, identifier: String, promise: Promise) throws {
    ...
    guard videoPresentation == nil, filePresentation == nil,
      ...
      throw NSError(
        domain: "KairoNativeVideo",
        code: 2,
        userInfo: [NSLocalizedDescriptionKey: "The video preview is no longer available."]
      )
    }
    let presentation = KairoNativeVideoPresentation(identifier: identifier, url: url, title: title) { [weak self] error in
  ...
  private func shareFile(url: URL, title: String, sourceIdentifier: String, promise: Promise) throws {
    guard let presenter = appContext?.utilities?.currentViewController() else {
      throw NSError(
        domain: "KairoNativePresentation",
        code: 2,
