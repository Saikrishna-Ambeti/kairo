...
public final class KairoKeyboardCommandsModule: Module {
  public func definition() -> ModuleDefinition {
    Name("KairoKeyboardCommands")

    View(KairoKeyboardCommandsView.self) {
      Prop("enabledCommands") { (view: KairoKeyboardCommandsView, commands: [String]) in
...
public final class KairoKeyboardCommandsView: ExpoView {
  ...
  @objc private func reclaimFirstResponderIfAvailable() {
    DispatchQueue.main.async { [weak self] in
      guard let self, self.window?.kairoFirstResponder == nil else { return }
...
private extension UIView {
  var kairoFirstResponder: UIResponder? {
    if isFirstResponder { return self }
    for subview in subviews {
      if let responder = subview.kairoFirstResponder { return responder }
    }
    return nil
  }
}
