...
final class KairoNativeFilePresentation: NSObject, QLPreviewControllerDataSource,
  QLPreviewControllerDelegate, UIAdaptivePresentationControllerDelegate {
  let identifier: String
  private var controller: UIViewController?
  private let completion: (Error?) -> Void
  private weak var sources: KairoPresentationSources?
  ...
  init(identifier: String, sources: KairoPresentationSources, sourceIdentifier: String, completion: @escaping (Error?) -> Void) {
  ...
  nonisolated private static func prepareFile(url: URL, title: String) async throws -> URL {
    ...
    let directory = FileManager.default.temporaryDirectory.appendingPathComponent("kairo-preview-\(UUID().uuidString)")
