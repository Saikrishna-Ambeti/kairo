...
final class KairoNativeVideoPresentation: NSObject, AVPlayerViewControllerDelegate,
  ...
  func present(from presenter: UIViewController, sources: KairoPresentationSources, sourceIdentifier: String) {
    let audioSession = AVAudioSession.sharedInstance()
    previousAudioSession = AudioSessionConfiguration(audioSession)
    do {
      try audioSession.setCategory(.playback, mode: .moviePlayback)
    } catch {
      NSLog("Kairo video audio session: %@", error.localizedDescription)
    }
