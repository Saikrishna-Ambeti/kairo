import type { EnvironmentId, ProviderInteractionMode, ServerProvider } from "@kairo/contracts";
...
import {
  ...
  scoreQueryMatch,
} from "@kairo/shared/searchRanking";
import {
...
export function buildComposerSlashCommandItems(input: {
  ...
  /** Whether Kairo itself offers /usage-limits for the selected provider. */
  ...
  for (const command of input.selectedProviderStatus?.slashCommands ?? []) {
    ...
    // Kairo's own limits command is answered by the thread composer; New Task has
