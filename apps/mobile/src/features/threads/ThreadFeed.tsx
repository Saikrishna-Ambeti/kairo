...
import type { EnvironmentId, MessageId, ThreadId, TurnId } from "@kairo/contracts";
...
import { resolveMarkdownLinkPresentation } from "@kairo/mobile-markdown-text/links";
import {
...
function renderFeedEntry(
  ...
  if (entry.type === "turn-fold") {
    return (
      ...
      >
        <Text className="font-kairo-medium text-sm tabular-nums text-foreground-muted">
          {entry.label}
        </Text>
        <SymbolView
      ...
      return (
        ...
        >
          ...
          <View className="mt-1 flex-row items-center justify-end gap-1 pr-0.5">
            <Text className="font-kairo-medium text-xs tabular-nums text-neutral-600 dark:text-neutral-400">
              {timestampLabel}
            </Text>
            {message.text.trim().length > 0 ? (
              <CopyTextButton
