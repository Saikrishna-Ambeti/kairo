...
import { hasProviderUsageLimits, isUsageLimitsCommand } from "@kairo/shared/usageLimits";
...
import { resolveProviderInteractionMode } from "./interaction-mode";
...
export function NewTaskDraftScreen(props: {
  ...
  const composerMenu = useComposerCommandMenu({
    ...
    onUpdateInteractionMode: flow.interactionModes.length > 1 ? flow.setInteractionMode : undefined,
  });
  ...
  async function handleStart(): Promise<void> {
    ...
    const interactionMode = resolveProviderInteractionMode(
      ...
      flow.interactionModes.includes(draft.interactionMode ?? flow.interactionMode)
    ...
    // Kairo's own limits command is answered by the thread composer; a new task would
  ...
  const hero = (
    <View className="items-center gap-6 px-6" testID="new-task-hero">
      <View className="w-full items-center gap-1.5">
        <Text className="text-center text-2xl font-kairo-medium tracking-tight text-foreground">
          What should we build
        </Text>
        <View className="max-w-full flex-row items-center justify-center">
          <Text className="text-2xl font-kairo-medium tracking-tight text-foreground">in </Text>
          <Pressable
          ...
          >
            <Text
              className="text-2xl font-kairo-medium tracking-tight text-foreground"
              numberOfLines={1}
            >
              {selectedProject.title}
            </Text>
          </Pressable>
          <Text className="text-2xl font-kairo-medium tracking-tight text-foreground">?</Text>
        </View>
      </View>
                      ...
                      <ComposerInlineControl
                        ...
                        onPress={() => {
                          ...
                          flow.setInteractionMode(
                            flow.interactionModes[
                              (currentIndex + 1) % flow.interactionModes.length
                            ] ?? "default",
                          );
                        }}
                        showChevron={false}
