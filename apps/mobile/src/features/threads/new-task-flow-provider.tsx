...
} from "@kairo/shared/threadEnvMode";
...
export function NewTaskFlowProvider(props: React.PropsWithChildren) {
  ...
  // checked-in kairo.json, then the server's configured default.
  const kairoProjectFileQuery = useEnvironmentQuery(
    selectedProject !== null && selectedProject.workspaceRoot !== ""
      ? projectEnvironment.readFile({
          environmentId: selectedProject.environmentId,
          input: { cwd: selectedProject.workspaceRoot, relativePath: KAIRO_PROJECT_FILE_NAME },
        })
      : null,
  );
  const kairoProjectFileData = kairoProjectFileQuery.data as ProjectReadFileResult | null;
  const kairoProjectFileDefaultMode = useMemo(() => {
    if (kairoProjectFileData === null || kairoProjectFileData.truncated) return null;
    return parseKairoProjectFile(kairoProjectFileData.contents)?.defaultThreadEnvMode ?? null;
  }, [kairoProjectFileData]);
  const defaultWorkspaceMode: WorkspaceMode = resolveDefaultThreadEnvMode({
    projectSetting: selectedProject?.defaultThreadEnvMode,
    projectFile: kairoProjectFileDefaultMode,
