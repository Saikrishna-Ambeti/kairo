...
} from "@kairo/client-runtime/state/shared-settings";
...
function ConfiguredSettingsRouteScreen({
    ...
    if (result.value.type === "granted") {
      ...
      if (getAgentAwarenessRegistrationStatus() === "registered") {
        ...
        Alert.alert(
          ...
          "Notification access was granted, but this device could not be registered with Kairo Connect. Notifications will start once registration succeeds.",
        );
      }
      return;
    }
  ...
  const promptSignIn = useCallback(() => {
    Alert.alert(
      ...
      "Live Activity updates require Kairo Connect so relay can deliver updates to this device.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Continue",
  ...
  const linkEnvironments = useCallback(async () => {
    ...
    if (getAgentAwarenessRegistrationStatus() === "registered") {
      ...
      Alert.alert(
        ...
        "This device could not be registered with Kairo Connect, so activity updates won't appear yet. They'll start once registration succeeds.",
      );
    }
  }, [
    connections,
  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        "Open system Settings to disable notifications for Kairo.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
    ...
    <>
      ...
      {mismatches.length > 0 ? (
        <View className="flex-row items-center gap-4 border-t border-border-subtle p-4">
          ...
          >
            <Text className="text-base font-kairo-medium text-foreground">Apply to all</Text>
          </Pressable>
        </View>
      ) : null}
    </>
