...
describe("mobileThemeRuntime", () => {
  it("keeps the default palette on Uniwind's built-in appearance themes", () => {
    expect(getMobileUniwindThemeName("kairo-code", "light")).toBe("light");
    expect(getMobileUniwindThemeName("kairo-code", "dark")).toBe("dark");
  });

  it("maps custom palettes and appearances to registered themes", () => {
    expect(getMobileUniwindThemeName("kairo-chat", "dark")).toBe("kairo-chat-dark");
  });
