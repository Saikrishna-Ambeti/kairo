...
describe("mobile connection storage", () => {
  ...
  it("preserves secure-storage read failures with operation and key context", async () => {
    ...
    await expect(loadSavedConnections()).rejects.toMatchObject({
      ...
      operation: "read",
      key: "kairo.connections",
      cause,
      message: "Mobile secure storage operation read failed for key kairo.connections.",
    });
  });

  it("logs structured decode failures before using the empty fallback", async () => {
    await mocks.setItemAsync("kairo.connections", "{");
    ...
    expect(warn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message: "Failed to decode mobile storage value for key kairo.connections.",
      }),
    );
  ...
  it("loads legacy preferences when SQLite is unavailable", async () => {
    ...
    await mocks.setItemAsync("kairo.preferences", JSON.stringify({ baseFontSize: 17 }));
  ...
  it("persists Material You independently for each appearance", async () => {
    ...
    await savePreferencesPatch({ lightThemeId: "kairo-chat" });
    await expect(loadPreferences()).resolves.toEqual({ ...themes, lightThemeId: "kairo-chat" });
  });
  ...
  it("falls back to secure storage when SQLite cannot save preferences", async () => {
    ...
    const fallback = JSON.parse(mocks.getStoredValue("kairo.preferences.fallback") ?? "") as {
  ...
  it("reconciles fallback preferences after SQLite recovers", async () => {
    ...
    expect(mocks.getStoredValue("kairo.preferences.fallback")).toBeNull();
  });
  ...
  it("keeps SQLite authoritative when stale legacy preferences remain", async () => {
    mocks.setPreferencesJson(JSON.stringify({ baseFontSize: 21 }), 30);
    await mocks.setItemAsync("kairo.preferences", JSON.stringify({ baseFontSize: 19 }));
