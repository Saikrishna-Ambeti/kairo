...
describe("owned attachment paths", () => {
  it.each([
    ...
    const oldUri = `${prefix}${OLD_CONTAINER}/Documents/kairo-composer-attachments/${FILE_NAME}`;
    const documentUri = `${prefix}${CURRENT_CONTAINER}/Documents/`;
    const currentUri = `${documentUri}kairo-composer-attachments/${FILE_NAME}`;
  ...
  it("recognizes the private/var alias without changing the stored filename", () => {
    const oldUri = `file:///private/var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/kairo-composer-attachments/${FILE_NAME}`;
  ...
  it.each([
    `file:///private/var/mobile/Containers/Shared/FileProvider/other/Documents/kairo-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Shared/AppGroup/other/kairo-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/report.pdf`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/kairo-composer-attachments/report.pdf`,
    `file:///downloads/kairo-composer-attachments/${FILE_NAME}`,
    `content://shared/kairo-composer-attachments/${FILE_NAME}`,
    `https://example.com/kairo-composer-attachments/${FILE_NAME}`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/kairo-composer-attachments/..%2F..%2Fsender.pdf`,
    `file:///var/mobile/Containers/Data/Application/${OLD_CONTAINER}/Documents/kairo-composer-attachments/${FILE_NAME}%2Fnested.pdf`,
