...
import { fromJsonStringPretty } from "@kairo/shared/schemaJson";
...
export class SqliteStateDatabaseMissingError extends Schema.TaggedErrorClass<SqliteStateDatabaseMissingError>()(
  ...
  override get message(): string {
    return `Database does not exist at '${this.databasePath}'. Start Kairo once to run migrations.`;
  }
}
...
export class SqliteStateSharedHomeMutationError extends Schema.TaggedErrorClass<SqliteStateSharedHomeMutationError>()(
  ...
  override get message(): string {
    return "Refusing to mutate the shared ~/.kairo database. Use an isolated --base-dir.";
  }
}
...
export const kairoSqliteStateCommand = Command.make(
  "kairo-sqlite-state",
  {
    ...
    baseDir: Flag.string("base-dir").pipe(
      Flag.withDescription("Explicit Kairo base directory containing userdata/state.sqlite."),
    ),
  ...
  Command.withDescription(
    "Inspect or seed an isolated Kairo SQLite database with automatic backups for writes.",
  ),
);

if (import.meta.main) {
  Command.run(kairoSqliteStateCommand, { version: "0.0.0" }).pipe(
