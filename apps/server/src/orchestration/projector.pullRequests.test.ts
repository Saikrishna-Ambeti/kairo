...
function makeLink(overrides: Partial<ThreadPullRequestLink> = {}): ThreadPullRequestLink {
  return {
    ...
    number: 42,
    url: "https://github.com/kairo/kairo/pull/42",
    source: "manual",
    linkedAt: NOW,
    snapshot: null,
    stack: null,
...
it.effect("ignores a sync for a pull request that is no longer linked", () =>
  Effect.gen(function* () {
    const created = yield* createThread(createEmptyReadModel(NOW));
    const other = makeLink({ number: 7, url: "https://github.com/kairo/kairo/pull/7" });
    const linked = yield* projectEvent(
      created,
      makeEvent({
        sequence: 2,
...
it.effect("mirrors legacy meta-updated links into pullRequests using the project host", () =>
  Effect.gen(function* () {
    ...
    const agentLink = makeLink({
      number: 7,
      url: "https://github.com/kairo/kairo/pull/7",
      source: "agent",
    });
      ...
      makeEvent({
        ...
        payload: {
          ...
          linkedPullRequest: {
            ...
            repository: "kairo/kairo",
            number: 42,
            url: "https://GitLab.example.com/kairo/kairo/-/merge_requests/42",
          },
          updatedAt: LATER,
        },
      }),
