...
it("keeps systemd pinned to the stable launcher rather than a versioned server", () => {
  const unit = BootService.renderBootServiceUnit({
    ...
    launcherPath: "/home/theo/.kairo/runtime/service-launcher.mjs",
    baseDir: "/home/theo/.kairo",
    logPath: "/home/theo/.kairo/userdata/logs/boot-service.log",
    unitPath: "/home/theo/.config/systemd/user/kairo.service",
  });

  expect(unit).toContain("ExecStart=/usr/bin/node /home/theo/.kairo/runtime/service-launcher.mjs");
...
it("keeps launchd pinned to the stable launcher rather than a versioned server", () => {
  ...
  expect(plist).toContain("<string>/Users/theo/.kairo/runtime/service-launcher.mjs</string>");
...
it("appends both stdio streams to the boot service log", () => {
  ...
  expect(plist).toContain(
    "<key>StandardOutPath</key>\n  <string>/Users/theo/.kairo/userdata/logs/boot-service.log</string>",
  );
  expect(plist).toContain(
...
it("escapes XML in host paths", () => {
  const plist = BootService.renderBootServicePlist(
    { ...macPlan, baseDir: "/Users/theo/Kairo & <Co>" },
    { homeDir: "/Users/theo", environmentPath: "/Users/theo/Tools & <Scripts>:/usr/bin" },
  );

  expect(plist).toContain("<string>/Users/theo/Kairo &amp; &lt;Co&gt;</string>");
...
const makeHarness = Effect.fn("test.make_boot_service_harness")(function* (
  ...
  const home = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-boot-service-test-" });
  const baseDir = path.join(home, ".kairo");
  ...
  const runner = ProcessRunner.ProcessRunner.of({
    run: Effect.fn("test.run_boot_service_command")(function* (
      ...
      if (!failed && command === "loginctl enable-linger --no-ask-password 501")
        control.linger = "yes";
      if (!failed && command === "systemctl --user enable kairo.service") control.enabled = true;
      if (!failed && command === "systemctl --user restart kairo.service") control.active = true;
      if (
        control.stateAfterStop !== undefined &&
        (command === "systemctl --user stop kairo.service" ||
...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(commands).not.toContain("systemctl --user stop kairo.service");
      }),
  );
  ...
  it.effect("installs, reports current state, and uninstalls", () =>
    Effect.gen(function* () {
      ...
      expect(timeouts.get("systemctl --user disable --now kairo.service")).toEqual(
    ...
    (platform) =>
      Effect.gen(function* () {
        ...
        expect(
          ...
          platform === "linux"
            ? ["systemctl --user stop kairo.service", "systemctl --user restart kairo.service"]
            : [
  ...
  it.effect("restarts an installed service when repair fails", () =>
    Effect.gen(function* () {
      ...
      expect(
        ...
        "systemctl --user stop kairo.service",
        "systemctl --user daemon-reload",
        "systemctl --user restart kairo.service",
      ]);
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        plan.unitPath.endsWith(
          path.join("Library", "LaunchAgents", "com.kairotools.kairo.service.plist"),
        ),
      ).toBe(true);
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install();
      control.failCommand = "launchctl bootout --wait gui/501/com.kairotools.kairo.service";
