...
it.layer(NodeServices.layer)("boot service install", (it) => {
  ...
  it.effect("installs, reports current state, and uninstalls on macOS", () =>
    Effect.gen(function* () {
      ...
      expect(plan.unitPath.endsWith("Library/LaunchAgents/com.kairo.app.service.plist")).toBe(true);
      expect(parseServiceState(yield* fs.readFileString(statePath))).toEqual({
        protocol: SERVICE_LAUNCHER_PROTOCOL,
        activeVersion: "1.2.3",
      });
  ...
  it.effect("ignores a bootout for an agent that is not loaded", () =>
    Effect.gen(function* () {
      ...
      yield* service.install;
      control.failCommand = "launchctl bootout --wait gui/501/com.kairo.app.service";
