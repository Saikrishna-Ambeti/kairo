...
/**
 * A pinned runtime is an exact `kairo@<version>` npm-installed into
 ...
 * switching over, never `npx kairo`, whose cache is ephemeral and whose
...
export function pinnedRuntimePaths(
  ...
  return {
    versionDir,
    entryPath: path.join(versionDir, "node_modules", "kairo", "dist", "bin.mjs"),
...
/**
 * Installs `kairo@<version>` into the pinned runtime directory unless a complete
...
const installPinnedRuntime = Effect.fn("cloud.pinned_runtime.ensure_installed")(function* (
  ...
  const stagingPaths: PinnedRuntimePaths = {
    ...
    entryPath: input.path.join(stagingDir, "node_modules", "kairo", "dist", "bin.mjs"),
    sentinelPath: input.path.join(stagingDir, ".install-complete"),
  };

  return yield* Effect.gen(function* () {
    const installStep = "installing the pinned kairo runtime (this can take a few minutes)";
    yield* runner
      .run({
        command: "npm",
        args: [
