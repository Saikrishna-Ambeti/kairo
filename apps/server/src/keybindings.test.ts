import { KeybindingCommand, KeybindingRule, KeybindingsConfig } from "@kairo/contracts";
...
import { KeybindingsConfigError } from "@kairo/contracts";
...
it.layer(NodeServices.layer)("keybindings", (it) => {
  ...
  it.effect("ships configurable thread navigation defaults", () =>
    Effect.sync(() => {
      ...
      assert.isFalse(defaultsByCommand.has("terminal.splitVertical"));
  ...
  it.effect("skips conflicting default keybindings on startup and logs a detailed warning", () => {
    ...
    return Effect.gen(function* () {
      const { keybindingsConfigPath } = yield* ServerConfig.ServerConfig;
      yield* writeKeybindingsConfig(keybindingsConfigPath, [
        { key: "mod+k", command: "script.custom-action.run" },
      ]);
      ...
      const persisted = yield* readKeybindingsConfig(keybindingsConfigPath);
      assert.isFalse(persisted.some((entry) => entry.command === "commandPalette.toggle"));
