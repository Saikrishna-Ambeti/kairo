...
describe("video asset byte ranges", () => {
  it.effect("uses current descriptor metadata after an in-place truncate or extension", () =>
    Effect.gen(function* () {
      ...
      const directory = yield* fs.makeTempDirectoryScoped({
        prefix: "kairo-guarded-current-stat-",
      });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const path = yield* Path.Path;
        const directory = yield* fs.makeTempDirectoryScoped({
          prefix: "kairo-guarded-offset-limit-",
        });
        const filePath = path.join(directory, "clip.mp4");
        yield* fs.writeFileString(filePath, "0123456789");
        const canonicalPath = yield* fs.realPath(filePath);
  ...
  it.effect("streams guarded file ranges, including suffixes and conditional requests", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-guarded-range-" });
      const filePath = path.join(directory, "clip.mp4");
      yield* fs.writeFileString(filePath, "0123456789");
      const canonicalPath = yield* fs.realPath(filePath);
  ...
  it.effect("closes guarded descriptors after full, HEAD, rejected, and cancelled responses", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-guarded-cleanup-" });
      const filePath = path.join(directory, "clip.mp4");
  ...
  it.effect("streams exactly the requested bytes and leaves full downloads intact", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const directory = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-video-range-" });
...
describe("downloadContentDisposition", () => {
  ...
  it("sets safe content disposition for document artifacts", () => {
    expect(
      assetResponseHeaders("/workspace/My Study Guide.docx", { disposition: "attachment" }),
    ).toHaveProperty("Content-Disposition", "attachment; filename*=UTF-8''My%20Study%20Guide.docx");
    expect(assetResponseHeaders("/workspace/guide.pdf", { disposition: "inline" })).toHaveProperty(
      "Content-Disposition",
