...
import { GrokSettings, ProviderInstanceId } from "@kairo/contracts";
...
const GrokTextGenerationTestLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "kairo-grok-text-generation-test-",
...
function withFakeAcpGrok<A, E, R>(
  ...
  return Effect.gen(function* () {
    const tempDir = mkdtempSync(path.join(os.tmpdir(), "kairo-grok-text-acp-"));
...
it.layer(GrokTextGenerationTestLayer)("GrokTextGeneration", (it) => {
  it.effect("uses ACP with disabled tool capabilities and forwards the requested model id", () => {
    const requestLogDir = mkdtempSync(path.join(os.tmpdir(), "kairo-grok-text-log-"));
  ...
  it.effect("surfaces ACP request failures as text generation errors", () =>
    withFakeAcpGrok(
      {
        Kairo_ACP_PROMPT_RESPONSE_TEXT: JSON.stringify({ branch: "unreachable" }),
      },
      (textGeneration) =>
        Effect.gen(function* () {
  ...
  it.effect("fails with TextGenerationError when output is unparseable JSON", () =>
    withFakeAcpGrok(
      {
        Kairo_ACP_PROMPT_RESPONSE_TEXT: "totally not json output from a confused model",
      },
      (textGeneration) =>
        Effect.gen(function* () {
          const error = yield* Effect.flip(
