...
describe("buildThreadTitlePrompt", () => {
  it("includes the user message and the title guidance rules", () => {
    ...
    expect(result.prompt).toContain(
      "Generate a title that will help the user recognize this Kairo thread weeks later.",
    );
    expect(result.prompt).toContain(
  ...
  it.each([
    ...
    ({ previousTitle }) => {
      const result = buildThreadTitlePrompt({
        message: "$takeover https://github.com/pingdotgg/kairo/pull/8588",
  ...
  it("regenerates from recent thread contents and identifies the previous title", () => {
    ...
    expect(result.prompt).toContain(
      "Regenerate the title for an existing Kairo thread so the user can recognize it weeks later.",
    );
