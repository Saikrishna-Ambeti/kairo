...
const makeFixture = Effect.fn("makeAntigravityTextGenerationFixture")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-antigravity-text-test-" });
...
it.layer(NodeServices.layer)("AntigravityTextGeneration", (it) => {
  ...
  it.effect("uses the native default without sending Kairo's default selection as a model ID", () =>
    Effect.gen(function* () {
      const fixture = yield* makeFixture();
