...
import { VcsProcessExitError, VcsProcessSpawnError } from "@kairo/contracts";
...
describe("GitHubCli.layer", () => {
  ...
  it.effect("keeps pull requests from gh versions without headRepository.nameWithOwner", () =>
    ...
    Effect.gen(function* () {
            ...
            JSON.stringify([
              {
                ...
                baseRefName: "main",
                headRefName: "kairo/codex-turn-mapping",
                state: "OPEN",
                mergedAt: null,
      ...
      const result = yield* gh.listOpenPullRequests({
        cwd: "/repo",
        headSelector: "kairo/codex-turn-mapping",
      });
  ...
  it.effect("creates repositories and parses clone URLs from create output", () =>
    Effect.gen(function* () {
      mockRun.mockReturnValueOnce(
        Effect.succeed(
          processOutput(
            "✓ Created repository octocat/kairo on github.com\nhttps://github.com/octocat/kairo\n",
          ),
        ),
      );
      ...
      expect(mockRun).toHaveBeenNthCalledWith(1, {
        operation: "GitHubCli.execute",
        command: "gh",
        args: ["repo", "create", "octocat/kairo", "--private"],
        cwd: "/repo",
        timeoutMs: 30_000,
      });
    }).pipe(Effect.provide(layer)),
