...
function errorDetail(error: ComposioServiceError): string | undefined {
  if (error._tag !== "ComposioError" || typeof error.cause !== "string") return undefined;
...
export const makeComposioService = Effect.gen(function* () {
  ...
  const probeCli: Effect.Effect<ComposioCliState, never> = Effect.gen(function* () {
    ...
    if (!executable) {
      return {
        status: hostPlatform === "win32" ? "unsupported" : "missing",
  ...
  const installCli = (operation: ComposioOperation): Effect.Effect<string, ComposioServiceError> =>
    Effect.gen(function* () {
      ...
      const result: ProcessRunOutput = yield* run(
        "bash",
        ["-lc", COMPOSIO_INSTALL_COMMAND],
        Duration.minutes(10),
      );
