...
const TestLayer = Layer.empty.pipe(
  Layer.provideMerge(KairoProjectFileLoader.layer),
...
const writeProjectFile = Effect.fn("writeProjectFile")(function* (cwd: string, contents: string) {
  ...
  yield* fileSystem.writeFileString(path.join(cwd, "kairo.json"), contents).pipe(Effect.orDie);
});

it.layer(TestLayer)("KairoProjectFileLoader", (it) => {
  describe("load", () => {
    it.effect("loads and decodes a valid kairo.json", () =>
      Effect.gen(function* () {
        const loader = yield* KairoProjectFileLoader.KairoProjectFileLoader;
    ...
    it.effect("returns none when kairo.json is missing", () =>
      Effect.gen(function* () {
        const loader = yield* KairoProjectFileLoader.KairoProjectFileLoader;
        const cwd = yield* makeTempDir;
