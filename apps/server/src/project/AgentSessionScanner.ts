...
} from "@kairo/shared/git";
import { HostProcessEnvironment, HostProcessPlatform } from "@kairo/shared/hostProcess";
import { normalizeProjectPathForComparison } from "@kairo/shared/path";
...
>()("kairo/project/AgentSessionScanner") {}
...
/**
 * Kairo runs its own agent sessions inside disposable worktrees. Their
 ...
 * worktrees directory plus the conventional `.kairo/worktrees` layout, which
 * also catches sandboxes from other Kairo homes on the same machine. Separators
...
function isT3ManagedWorktree(
  ...
  return (
    ...
    normalized.includes("/.kairo/worktrees/")
  );
}
