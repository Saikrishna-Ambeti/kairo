...
it.layer(NodeServices.layer)("RepositoryIdentityResolverLive", (it) => {
        ...
        Effect.sync(() => {
          ...
          return {
            stdout: input.args.includes("rev-parse")
              ? `${rootPath}\n`
              : "origin\tgit@github.com:kairo/kairo.git (fetch)\n",
            stderr: "",
  ...
  it.effect("normalizes equivalent GitHub remotes into a stable repository identity", () =>
    Effect.gen(function* () {
      ...
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@github.com:Kairo/kairo.git"]);
      ...
      expect(identity?.canonicalKey).toBe("github.com/kairo/kairo");
      expect(normalizeResolvedPath(resolvedIdentityRoot)).toBe(normalizeResolvedPath(resolvedCwd));
      expect(identity?.displayName).toBe("kairo/kairo");
      expect(identity?.provider).toBe("github");
      expect(identity?.owner).toBe("kairo");
      expect(identity?.name).toBe("kairo");
  ...
  it.effect("returns the git top-level root path when resolving from a nested workspace", () =>
    Effect.gen(function* () {
      ...
      const repoRoot = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-nested-root-test-",
      });
      ...
      yield* git(repoRoot, ["remote", "add", "origin", "git@github.com:Kairo/kairo.git"]);
  ...
  it.effect("returns null for non-git folders and repos without remotes", () =>
    Effect.gen(function* () {
      ...
      const nonGitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-non-git-",
      });
      const gitDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-no-remote-",
      });
  ...
  it.effect.each(["add", "replace"] as const)(
    ...
    (change) =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "kairo-repository-identity-upstream-test-",
        });

        yield* git(cwd, ["init"]);
        yield* git(cwd, ["remote", "add", "origin", "git@github.com:julius/kairo.git"]);
        if (change === "replace") {
          yield* git(cwd, ["remote", "add", "upstream", "git@github.com:kairo/previous.git"]);
        }

        const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
        const initialIdentity = yield* resolver.resolve(cwd);
        expect(initialIdentity?.canonicalKey).toBe(
          change === "add" ? "github.com/julius/kairo" : "github.com/kairo/previous",
        );
        ...
        yield* git(cwd, [
          ...
          "upstream",
          "git@github.com:kairo/kairo.git",
        ]);
  ...
  it.effect("uses the last remote path segment as the repository name for nested groups", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-nested-group-test-",
      });

      yield* git(cwd, ["init"]);
      yield* git(cwd, ["remote", "add", "origin", "git@gitlab.com:Kairo/platform/kairo.git"]);

      const resolver = yield* RepositoryIdentityResolver.RepositoryIdentityResolver;
      const identity = yield* resolver.resolve(cwd);

      expect(identity).not.toBeNull();
      expect(identity?.canonicalKey).toBe("gitlab.com/kairo/platform/kairo");
      expect(identity?.displayName).toBe("kairo/platform/kairo");
      expect(identity?.owner).toBe("kairo");
      expect(identity?.name).toBe("kairo");
    }).pipe(Effect.provide(RepositoryIdentityResolver.layer)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const cwd = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "kairo-repository-identity-late-remote-test-",
        });
        ...
        expect(refreshedIdentity?.canonicalKey).toBe("github.com/kairo/kairo");
        expect(refreshedIdentity?.name).toBe("kairo");
      }).pipe(
        Effect.provide(
          Layer.merge(
            TestClock.layer(),
  ...
  it.effect("refreshes cached identities after the positive TTL when a remote changes", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const cwd = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "kairo-repository-identity-remote-change-test-",
      });
      ...
      expect(initialIdentity?.canonicalKey).toBe("github.com/kairo/kairo");

      yield* git(cwd, ["remote", "set-url", "origin", "git@github.com:Kairo/kairo-next.git"]);

      const cachedIdentity = yield* resolver.resolve(cwd);
      expect(cachedIdentity).not.toBeNull();
      expect(cachedIdentity?.canonicalKey).toBe("github.com/kairo/kairo");

      yield* TestClock.adjust(Duration.millis(180));

      const refreshedIdentity = yield* resolver.resolve(cwd);
      expect(refreshedIdentity).not.toBeNull();
      expect(refreshedIdentity?.canonicalKey).toBe("github.com/kairo/kairo-next");
      expect(refreshedIdentity?.displayName).toBe("kairo/kairo-next");
      expect(refreshedIdentity?.name).toBe("kairo-next");
    }).pipe(
      Effect.provide(
        Layer.merge(
          TestClock.layer(),
