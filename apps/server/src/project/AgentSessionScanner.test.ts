...
import { symlinksSupported } from "@kairo/shared/testing/symlinks";
    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        ServerConfig.layerTest(
          ...
          input.configBaseDir ?? { prefix: "kairo-scanner-config-" },
        ),
...
it.layer(NodeServices.layer)("AgentSessionScanner", (it) => {
  describe("scan", () => {
    it.effect("reads Claude project cwds from transcripts, newest first", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const olderWorkspace = yield* makeTempDir("kairo-workspace-older-");
        const newerWorkspace = yield* makeTempDir("kairo-workspace-newer-");
    ...
    it.effect("groups Codex rollouts by cwd across date directories", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const workspace = yield* makeTempDir("kairo-workspace-");
        const otherWorkspace = yield* makeTempDir("kairo-workspace-other-");
    ...
    it.effect("returns the imported project ID through a realpath alias", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const workspace = yield* makeTempDir("kairo-workspace-");
        const linkParent = yield* makeTempDir("kairo-scanner-links-");
    ...
    it.effect("keeps case variants distinct when the filesystem identities differ", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const backingUpper = yield* makeTempDir("kairo-backing-upper-");
        const backingLower = yield* makeTempDir("kairo-backing-lower-");
        const aliasParent = yield* makeTempDir("kairo-case-aliases-");
    ...
    it.effect("uses explicit provider instance homes instead of overridden legacy homes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("kairo-claude-legacy-");
        const codexHomePath = yield* makeTempDir("kairo-codex-legacy-");
        const claudeInstanceHome = yield* makeTempDir("kairo-claude-instance-");
        const codexInstanceHome = yield* makeTempDir("kairo-codex-instance-");
        const legacyWorkspace = yield* makeTempDir("kairo-workspace-legacy-");
        const claudeWorkspace = yield* makeTempDir("kairo-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("kairo-workspace-codex-");
    ...
    it.effect("scans each distinct home across multiple instances once", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const otherCodexHome = yield* makeTempDir("kairo-codex-other-");
        const workspace = yield* makeTempDir("kairo-workspace-");
        const otherWorkspace = yield* makeTempDir("kairo-workspace-other-");
    ...
    it.effect("honors provider instance home directory environment variables", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-legacy-");
        const codexHomePath = yield* makeTempDir("kairo-codex-legacy-");
        const claudeEnvironmentHome = yield* makeTempDir("kairo-claude-env-");
        const codexEnvironmentHome = yield* makeTempDir("kairo-codex-env-");
        const claudeWorkspace = yield* makeTempDir("kairo-workspace-claude-");
        const codexWorkspace = yield* makeTempDir("kairo-workspace-codex-");
    ...
    it.effect("does not scan provider instances disabled by the envelope or config", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const envelopeDisabledHome = yield* makeTempDir("kairo-codex-disabled-envelope-");
        const configDisabledHome = yield* makeTempDir("kairo-codex-disabled-config-");
        const envelopeWorkspace = yield* makeTempDir("kairo-workspace-disabled-envelope-");
        const configWorkspace = yield* makeTempDir("kairo-workspace-disabled-config-");
    ...
    it.effect("excludes the home directory, temporary root, and Kairo data directory", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const configBaseDir = yield* makeTempDir("kairo-scanner-base-");
        const workspace = yield* makeTempDir("kairo-workspace-");
    ...
    it.effect("excludes Kairo-managed worktree sandboxes", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        ...
        const worktreeCwd = path.join(claudeHomePath, ".kairo", "worktrees", "kairo", "wt-1");
    ...
    it.effect("excludes Codex scratch directories and Downloads", () =>
      Effect.gen(function* () {
        ...
        const runId = path.basename(yield* makeTempDir("kairo-scanner-test-"));
        ...
        const keep = yield* makeTempDir("kairo-workspace-keep-");
    ...
    it.effect("skips linked git worktrees and reports the origin of real checkouts", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const repo = yield* makeTempDir("kairo-workspace-repo-");
        const worktree = yield* makeTempDir("kairo-workspace-worktree-");
        const plain = yield* makeTempDir("kairo-workspace-plain-");
        const noRemote = yield* makeTempDir("kairo-workspace-noremote-");
        const submodule = yield* makeTempDir("kairo-workspace-submodule-");

        yield* fileSystem.makeDirectory(path.join(repo, ".git"));
        yield* fileSystem.writeFileString(
          path.join(repo, ".git", "config"),
          '[core]\n\tbare = false\n[remote "origin"]\n\turl = git@github.com:pingdotgg/kairo.git\n\tfetch = +refs/heads/*:refs/remotes/origin/*\n',
        );
        yield* fileSystem.writeFileString(
        ...
        expect(
          ...
          {
            path: repo,
            git: { remoteKey: "github.com/pingdotgg/kairo", repository: "pingdotgg/kairo" },
          },
        ]);
      }),
    );

    it.effect("excludes sandboxes under the configured worktrees dir without .kairo in the path", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        ...
        // dir contains no `.kairo` segment — only the config-based prefix match
        // can exclude this one.
        const worktreeCwd = path.join(configBaseDir, "worktrees", "kairo", "wt-2");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
        yield* writeTranscript({
          filePath: path.join(claudeHomePath, "projects", "-slug", "a.jsonl"),
          contents: claudeSessionLine(worktreeCwd),
    ...
    it.effect("excludes sandboxes reached through a symlink into the worktrees dir", () =>
      Effect.gen(function* () {
        ...
        const worktreeCwd = path.join(configBaseDir, "worktrees", "kairo", "wt-3");
        yield* fileSystem.makeDirectory(worktreeCwd, { recursive: true });
    ...
    it.effect.each([64, 65])("shares metadata bytes across homes for %s one-MiB files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("kairo-metadata-home-");
        const secondHome = yield* makeTempDir("kairo-metadata-second-");
        const codexHomePath = yield* makeTempDir("kairo-metadata-codex-");
        const firstWorkspace = yield* makeTempDir("kairo-metadata-first-project-");
        const secondWorkspace = yield* makeTempDir("kairo-metadata-second-project-");
        const directories = [
    ...
    it.effect.each([50, 51])("bounds metadata open/read calls for %s short-read files", (count) =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("kairo-short-metadata-home-");
        const codexHomePath = yield* makeTempDir("kairo-short-metadata-codex-");
        const workspace = yield* makeTempDir("kairo-short-metadata-project-");
    ...
    it.effect("bounds malformed metadata records without excluding another account", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const claudeHomePath = yield* makeTempDir("kairo-record-metadata-home-");
        const secondHome = yield* makeTempDir("kairo-record-metadata-second-");
        const codexHomePath = yield* makeTempDir("kairo-record-metadata-codex-");
        const workspace = yield* makeTempDir("kairo-record-metadata-project-");
        const directory = path.join(claudeHomePath, "projects", "p");
        const template = path.join(directory, "template.jsonl");
        yield* writeTranscript({
          filePath: template,
    ...
    it.effect.each([19_999, 20_000])(
      ...
      (count) =>
        Effect.gen(function* () {
          const path = yield* Path.Path;
          const fileSystem = yield* FileSystem.FileSystem;
          const claudeHomePath = yield* makeTempDir("kairo-directory-budget-home-");
          const codexHomePath = yield* makeTempDir("kairo-directory-budget-codex-");
    ...
    it.effect("returns an empty result when neither home directory exists", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const root = yield* makeTempDir("kairo-missing-homes-");
  ...
  describe("recentThreads", () => {
    it.effect.each([false, true])(
      ...
      (overflow) =>
        Effect.gen(function* () {
          ...
          const claudeHomePath = yield* makeTempDir("kairo-record-limit-claude-");
          const codexHomePath = yield* makeTempDir("kairo-record-limit-codex-");
          const workspace = yield* makeTempDir("kairo-record-limit-project-");
    ...
    it.effect("keeps the provider instance that owns a custom session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const customHome = yield* makeTempDir("kairo-codex-custom-");
        const workspace = yield* makeTempDir("kairo-workspace-");
    ...
    it.effect("streams large transcripts across providers without hiding projects", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("kairo-budget-claude-");
        const codexHomePath = yield* makeTempDir("kairo-budget-codex-");
        const workspace = yield* makeTempDir("kairo-budget-workspace-");
    ...
    it.effect("skips excessive records without blocking an older valid transcript", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("kairo-record-budget-claude-");
        const codexHomePath = yield* makeTempDir("kairo-record-budget-codex-");
        const workspace = yield* makeTempDir("kairo-record-budget-workspace-");
        ...
        it.effect.skipIf(replacement === "symlink alias" && !symlinksSupported)(
          ...
          () =>
            Effect.gen(function* () {
              ...
              const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
              yield* TestClock.setTime(nowMs);
              const fixture = yield* makeTempDir("kairo-replaced-cwd-");
    ...
    it.effect("checks file identity and provider before skipping completed history", () =>
      Effect.gen(function* () {
        ...
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        yield* TestClock.setTime(nowMs);
        const claudeHomePath = yield* makeTempDir("kairo-completed-claude-");
        const codexHomePath = yield* makeTempDir("kairo-completed-codex-");
        const workspace = yield* makeTempDir("kairo-completed-workspace-");
    ...
    it.effect("does not reopen a transcript that becomes a non-file after discovery", () =>
      Effect.gen(function* () {
        ...
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const workspace = yield* makeTempDir("kairo-workspace-");
        const nonFilePath = yield* makeTempDir("kairo-non-file-");
    ...
    it.effect("does not import sessions from a Kairo-managed worktree", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const fileSystem = yield* FileSystem.FileSystem;
        const nowMs = Date.parse("2026-08-24T12:00:00.000Z");
        ...
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const configBaseDir = yield* makeTempDir("kairo-scanner-base-");
        const workspace = path.join(configBaseDir, "worktrees", "kairo", "managed-worktree");
    ...
    it.effect("uses one deterministic provider instance for a shared session home", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const sharedHome = yield* makeTempDir("kairo-codex-shared-");
        const workspace = yield* makeTempDir("kairo-workspace-");
    ...
    it.effect("keeps a second account when the first has 5000 newer files", () =>
      Effect.gen(function* () {
        ...
        const claudeHomePath = yield* makeTempDir("kairo-claude-home-");
        const codexHomePath = yield* makeTempDir("kairo-codex-home-");
        const oldWorkspace = yield* makeTempDir("kairo-workspace-old-");
        const recentWorkspace = yield* makeTempDir("kairo-workspace-recent-");
        const recentHome = yield* makeTempDir("kairo-claude-recent-home-");
