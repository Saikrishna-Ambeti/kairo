 ...
 * owns that path; anything unproven stays manual-only so Kairo never runs
...
export function makePackageManagedProviderMaintenanceResolver(
  ...
  return {
    resolve: (context) =>
      Effect.gen(function* () {
        const capabilities = yield* resolvePackageManagedProviderMaintenance(definition, context);
        const platform = context?.platform ?? (yield* HostProcessPlatform);
        if (!definition.login) return capabilities;
        const executable = context?.resolvedCommandPath ?? definition.login.executable;
        return {
          ...capabilities,
          login: {
            executable,
            args: definition.login.args,
            lockKey: definition.login.lockKey,
            command: [quoteUpdateExecutable(executable, platform), ...definition.login.args].join(
              " ",
            ),
          },
        };
