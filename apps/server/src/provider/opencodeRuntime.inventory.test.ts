...
it.layer(testLayer)("OpenCodeRuntime inventory", (it) => {
  ...
  it.effect("drops oversized CLI skill output without losing the model inventory", () =>
    Effect.gen(function* () {
      ...
      const tempDir = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-opencode-inventory-" });
      ...
      yield* fs.writeFileString(
        ...
        [
          ...
          isWindows
            ? '"%Kairo_TEST_NODE_BINARY%" "%Kairo_TEST_OPENCODE_SCRIPT%" %*'
            : 'exec "$Kairo_TEST_NODE_BINARY" "$Kairo_TEST_OPENCODE_SCRIPT" "$@"',
          "",
        ].join("\n"),
      );
      if (!isWindows) {
      ...
      const inventory = yield* runtime.loadInventoryFromCli({
        ...
        environment: {
          ...hostEnvironment,
          Kairo_TEST_NODE_BINARY: executablePath,
          Kairo_TEST_OPENCODE_SCRIPT: scriptPath,
        },
      });
