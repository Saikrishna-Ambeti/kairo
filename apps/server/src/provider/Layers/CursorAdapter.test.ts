...
class CursorAdapter extends Context.Service<CursorAdapter, CursorAdapterShape>()(
  "kairo/provider/Layers/CursorAdapter.test/CursorAdapter",
) {}
...
async function makeProbeWrapper(
  ...
  return writeFakeCli({
    directory: dir,
    name: "fake-agent",
    env: { KAIRO_ACP_REQUEST_LOG_PATH: requestLogPath, ...extraEnv },
  ...
  Layer.effect(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "kairo-cursor-adapter-test-",
      }),
    ),
...
cursorAdapterTestLayer("CursorAdapterLive", (it) => {
    ...
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({
          KAIRO_ACP_PROMPT_RESPONSE_TEXT: "Error: RetriableError: WritableIterable is closed",
        }),
      );
  ...
  it.effect("steers a running turn instead of opening a new one on mid-turn sendTurn", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ KAIRO_ACP_PROMPT_DELAY_MS: "1500" }),
      );
      yield* settings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const previousEmitToolCalls = process.env.KAIRO_ACP_EMIT_TOOL_CALLS;
        process.env.KAIRO_ACP_EMIT_TOOL_CALLS = "1";
        ...
        const wrapperPath = yield* Effect.promise(() =>
          makeMockAgentWrapper({ KAIRO_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
          ...
          Effect.ensuring(
            Effect.sync(() => {
              if (previousEmitToolCalls === undefined) {
                delete process.env.KAIRO_ACP_EMIT_TOOL_CALLS;
              } else {
                process.env.KAIRO_ACP_EMIT_TOOL_CALLS = previousEmitToolCalls;
              }
            }),
          ),
        );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() => NodeFSP.writeFile(requestLogPath, "", "utf8"));
        const wrapperPath = yield* Effect.promise(() =>
          makeProbeWrapper(requestLogPath, argvLogPath, { KAIRO_ACP_EMIT_TOOL_CALLS: "1" }),
        );
        yield* serverSettings.updateSettings({
          providers: { cursor: { binaryPath: wrapperPath } },
        });
  ...
  it.effect("segments assistant messages around ACP tool activity in full-access mode", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ KAIRO_ACP_EMIT_INTERLEAVED_ASSISTANT_TOOL_CALLS: "1" }),
      );
      yield* serverSettings.updateSettings({
        providers: { cursor: { binaryPath: wrapperPath } },
      });
  ...
  it.effect("stopping a session settles pending user-input waits", () =>
    Effect.gen(function* () {
      ...
      const wrapperPath = yield* Effect.promise(() =>
        makeMockAgentWrapper({ KAIRO_ACP_EMIT_ASK_QUESTION: "1" }),
      );
      yield* serverSettings.updateSettings({ providers: { cursor: { binaryPath: wrapperPath } } });
  ...
  it.effect(
    ...
    () => {
      ...
      const customAdapterLayer = Layer.effect(
        ...
        Layer.provideMerge(
          ServerConfig.layerTest(process.cwd(), {
            prefix: "kairo-cursor-adapter-custom-instance-",
          }),
        ),
        Layer.provideMerge(NodeServices.layer),
      );
