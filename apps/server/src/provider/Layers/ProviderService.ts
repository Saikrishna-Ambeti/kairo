...
import { expandAssistantCitationsForProvider } from "@kairo/shared/assistantCitations";
import { causeErrorTag } from "@kairo/shared/observability";
import { getModelSelectionStringOptionValue } from "@kairo/shared/model";
import { resolveProjectAgentBrowserAccess } from "@kairo/shared/serverSettings";
...
export interface ProviderServiceLiveOptions {
   ...
   * MCP capability selection unobservable from a unit test; this seam lets a
   * test see whether a credential was requested and which capabilities it held.
   */
...
const makeProviderService = Effect.fn("makeProviderService")(function* (
  ...
  const resolveMcpCapabilities = (providerInstanceId: ProviderInstanceId) =>
    serverSettings.getSettings.pipe(
      Effect.flatMap((settings) => {
        ...
        if (!memory.enabled || !memory.providerInstanceIds.includes(providerInstanceId)) {
          return Effect.succeed(capabilities);
        }
        return Effect.serviceOption(ServerSecretStore.ServerSecretStore).pipe(
          Effect.flatMap((secretStore) =>
            Option.isSome(secretStore)
              ? getKairoCloudAccessToken().pipe(
                  Effect.provideService(ServerSecretStore.ServerSecretStore, secretStore.value),
                  Effect.map((accessToken) => {
                    if (accessToken) capabilities.add("memory");
                    return capabilities;
                  }),
                  Effect.orElseSucceed(() => capabilities),
                )
      ...
      Effect.catch((cause) =>
        Effect.logWarning("Could not resolve MCP capabilities for this provider session.", {
          cause,
  ...
  /**
   * Attach the `kairo` MCP server to the session that is about to start.
   *
  ...
  const prepareMcpSession = (threadId: ThreadId, providerInstanceId: ProviderInstanceId) =>
    Effect.gen(function* () {
      const capabilities = yield* resolveMcpCapabilities(providerInstanceId);
    ...
    function* (threadId, rawInput) {
      ...
      return yield* Effect.gen(function* () {
        ...
        if (!instanceInfo.enabled) {
          return yield* toValidationError(
            "ProviderService.startSession",
            `Provider instance '${resolvedInstanceId}' is disabled in Kairo settings.`,
          );
        }
  ...
  const sendTurn: ProviderServiceMethod<"sendTurn"> = Effect.fn("sendTurn")(function* (rawInput) {
    ...
    return yield* Effect.gen(function* () {
      ...
      yield* analytics.record("provider.turn.sent", {
        ...
        interactionMode: input.interactionMode,
        deepResearch: resolveDeepResearchProviderInput(parsed.input).deepResearch,
