...
const makeHarness = Effect.fn("makeAntigravityDriverHarness")(function* (
  ...
  const root = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-antigravity-driver-" });
  ...
  const instance = yield* AntigravityDriver.create({
    ...
    environment: [
      ...
      { name: "Kairo_ACP_ANTIGRAVITY", value: "1" },
      { name: "Kairo_ACP_REQUEST_LOG_PATH", value: requestLog },
...
const testLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "kairo-antigravity-driver-config-",
}).pipe(
