import { AntigravitySettings, ProviderDriverKind, ProviderSetupError } from "@kairo/contracts";
    ...
    Effect.gen(function* () {
      ...
      const makeDisposableRuntime = Effect.fn("AntigravityDriver.makeDisposableRuntime")(function* (
        ...
        const cwd = yield* fileSystem
          .makeTempDirectoryScoped({ prefix: "kairo-antigravity-setup-" })
          .pipe(
            Effect.mapError(
              () =>
                new ProviderSetupError({
        ...
        const runtime = yield* makeRuntime({
          cwd,
          clientInfo: { name: "kairo-provider-setup", version: "0.0.0" },
          mcpServers: [],
      ...
      const textGeneration = yield* makeAntigravityTextGeneration({
        ...
        makeRuntime: (cwd) =>
          makeRuntime({
            cwd,
            clientInfo: { name: "kairo-text", version: "0.0.0" },
            mcpServers: [],
          }),
      });
