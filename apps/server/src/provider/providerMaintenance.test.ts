...
it.layer(NodeServices.layer)("providerMaintenance", (it) => {
  ...
  it.effect("proves Windows npm ownership from the package manifest beside the shim", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("kairo-npm-windows-capabilities");
  ...
  it.effect.skipIf(windowsHost)("switches to native updates and runs the resolved executable", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("kairo-native-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    "keeps login on the selected native install alongside update environment",
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("kairo-native-login");
        const nativePath = NodePath.join(tempDir, "selected tool");
        writeExecutable(nativePath);
        const resolver = makePackageManagedProviderMaintenanceResolver({
          ...
          login: {
            ...
            args: ["auth", "login"],
            lockKey: "provider-login",
          },
          nativeUpdate: {
            args: ["update"],
            isCommandPath: () => true,
            env: { INSTALL_ROOT: tempDir },
          },
        });
        ...
        expect(capabilities.login).toMatchObject({
          executable: nativePath,
          args: ["auth", "login"],
          lockKey: "provider-login",
        });
  ...
  it.effect.skipIf(windowsHost)("runs an explicit native updater outside PATH", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("kairo-native-update");
  ...
  it.effect.skipIf(!symlinksSupported)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const tempDir = yield* makeTempDir("kairo-homebrew-node-capabilities");
  ...
  it.effect.skipIf(windowsHost)("carries the native updater's environment into the action", () =>
    Effect.gen(function* () {
      const tempDir = yield* makeTempDir("kairo-native-env");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("kairo-missing-native-capabilities");
  ...
  it.effect.each([
    ...
    (fixture) =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("kairo-homebrew-capabilities");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("kairo-homebrew-foreign-prefix");
        const brewBinDir = NodePath.join(tempDir, "brew-bin");
  ...
  it.effect.skipIf(windowsHost)(
    ...
    () =>
      Effect.gen(function* () {
        const tempDir = yield* makeTempDir("kairo-custom-capabilities");
