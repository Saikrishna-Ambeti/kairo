...
describe("grokAcpSpawnArgs", () => {
  it("inherits the Grok CLI config when no Kairo runtime mode is set", () => {
    expect(grokAcpSpawnArgs()).toEqual(["agent", "stdio"]);
  });

  it("forces Grok to ask when Kairo is Supervised", () => {
...
describe("buildGrokAcpSpawnInput", () => {
  it("passes the Kairo referrer through Grok OAuth env", () => {
