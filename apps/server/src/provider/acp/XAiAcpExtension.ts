...
import type { ProviderUserInputAnswers, UserInputQuestion } from "@kairo/contracts";
...
/**
 * Client captured the plan for Kairo's proposed-plan card. Abandon the native
 * Grok plan-approval gate so the turn unblocks; the user implements via Kairo UI.
 */
 ...
 * Used so Kairo can show the plan while plan mode is still active (before exit).
 */
...
export const makeXAiPromptCompletionRuntime = Effect.fn("makeXAiPromptCompletionRuntime")(
  function* (runtime: AcpSessionRuntime.AcpSessionRuntime["Service"]) {
    ...
    const allocatePromptFallbackId = Effect.sync(() => {
      ...
      return `kairo-xai-prompt-${nextPromptFallbackId}`;
    });
