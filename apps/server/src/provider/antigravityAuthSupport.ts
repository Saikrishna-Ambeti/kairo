...
import type { AntigravityAuthMethod, ProviderInstanceId } from "@kairo/contracts";
import { HostProcessExecutablePath, HostProcessPlatform } from "@kairo/shared/hostProcess";
...
export const ANTIGRAVITY_AUTH_BROWSER_MARKER = "__Kairo_ANTIGRAVITY_AUTH_URL__";
...
const browserHelperSource =
  ...
  `()=>process.exit(0))`;
const browserPreflightUrl = "https://example.invalid/kairo-antigravity-browser-preflight";
...
/** Recognizes native auth failures and interactive login blocked by Kairo. */
...
/**
 * The agent reads its user-global skills under `GEMINI_HOME`, which Kairo points
...
export const prepareAntigravityProfile = Effect.fn("prepareAntigravityProfile")(function* (input: {
  ...
  if (
    ...
    return yield* authSupportError(
      "The Kairo runtime path cannot be used to suppress Antigravity browser launches.",
    );
  }
...
/** Receives native 1.1.1 sign-in URLs and Kairo browser-helper URLs without logging stderr. */
