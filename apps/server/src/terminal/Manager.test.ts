...
> =>
  Effect.flatMap(Effect.service(FileSystem.FileSystem), (fs) =>
    Effect.gen(function* () {
      ...
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-terminal-" });
...
it.layer(
  ...
  it.effect("attaches to exited sessions without restarting them", () =>
    Effect.gen(function* () {
      ...
      const unsubscribe = yield* manager.attachStream(
        openInput({
          env: {
            KAIRO_WORKTREE_PATH: "/tmp/should-not-restart",
          },
      ...
      const unsubscribe = yield* manager.attachStream(
        {
          ...openInput({
            env: {
              KAIRO_WORKTREE_PATH: "/tmp/restart-requested",
            },
  ...
  it.effect("filters app runtime env variables from terminal sessions", () =>
    Effect.gen(function* () {
      ...
      expect(spawnInput.env.KAIRO_PORT).toBeUndefined();
  ...
  it.effect("strips AppImage runtime env from terminal sessions", () =>
    Effect.gen(function* () {
      const appDir = "/tmp/.mount_Kairoabc123";
      const { manager, ptyAdapter } = yield* createManager(5, {
        env: {
          APPIMAGE: "/home/user/Kairo-Code.AppImage",
          APPDIR: appDir,
          ARGV0: "/home/user/Kairo-Code.AppImage",
  ...
  it.effect("injects runtime env overrides into spawned terminals", () =>
    Effect.gen(function* () {
      ...
      yield* manager.open(
        openInput({
          env: {
            KAIRO_PROJECT_ROOT: "/repo",
            KAIRO_WORKTREE_PATH: "/repo/worktree-a",
            CUSTOM_FLAG: "1",
          },
        }),
      );
      ...
      assert.equal(spawnInput.env.KAIRO_PROJECT_ROOT, "/repo");
      assert.equal(spawnInput.env.KAIRO_WORKTREE_PATH, "/repo/worktree-a");
  ...
  it.effect("resolves a provider instance environment before spawning", () =>
    Effect.gen(function* () {
      ...
      expect(ptyAdapter.spawnInputs[0]?.env.KAIRO_SECRET).toBeUndefined();
    ...
    Effect.gen(function* () {
      ...
      Effect.provide(
        ServerSettings.layer.pipe(
          ...
          Layer.provide(
            ServerConfig.layerTest(process.cwd(), { prefix: "kairo-terminal-provider-restart-" }),
          ),
        ),
      ),
    ),
