...
/**
 * `kairo theme` - inspect and set the environment's theme. Connected web and
 ...
 * Settings afterwards sticks until the next `kairo theme set`.
 *
...
import { fromJsonStringPretty, fromLenientJson } from "@kairo/shared/schemaJson";
import { BUILT_IN_THEME_IDS, UNPUBLISHABLE_THEME_IDS } from "@kairo/shared/themePalettes";
...
export class ThemeFileInvalidError extends Schema.TaggedErrorClass<ThemeFileInvalidError>()(
  ...
  override get message(): string {
    return `${this.filePath} is not a valid theme file. Use a theme exported from Kairo, or a seeded file with name, appearance, canvas, and accent.`;
  }
}
...
export class ThemeTargetMissingError extends Schema.TaggedErrorClass<ThemeTargetMissingError>()(
  ...
  override get message(): string {
    return "Provide a theme id or file, or run `kairo theme clear` to remove the theme.";
  }
}

const envKairoHome = Config.string("KAIRO_HOME").pipe(Config.option);

const resolveThemePaths = Effect.fn(function* (explicitBaseDir: Option.Option<string>) {
  // Same precedence as the rest of the CLI: --base-dir, then KAIRO_HOME,
  // then the default home. A provisioning script exporting KAIRO_HOME must
  // not have this one command silently target the default install.
  const envHome = Option.filter(yield* envKairoHome, (value) => value.trim().length > 0);
...
const themeSetCommand = Command.make("set", {
  ...
  Command.withHandler((flags) =>
    Effect.gen(function* () {
      ...
      // make `kairo theme set ocean` publish ./ocean whenever the cwd happens to
