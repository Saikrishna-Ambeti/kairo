...
import { HostProcessArguments } from "@kairo/shared/hostProcess";
 ...
 * Detection is best-effort; callers must fail closed to a plain `kairo` command.
 */
...
/**
 * The `kairo` package spec to suggest. The literal spec the user typed (e.g.
 * `kairo@nightly`) is resolved away before our process starts, so re-derive it
...
export function suggestedPackageSpec(version: string): string {
  return version.includes("-nightly.") ? "kairo@nightly" : "kairo";
}

/**
 * Render a `kairo <subcommand>` suggestion that matches how this process was
 * launched, so copy/pasting it actually works: `npx kairo connect` suggests
 * `npx kairo serve`, a global install suggests `kairo serve`, and a nightly build
