...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (liveResult.status === "failed") {
    yield* Console.warn(
      `Kairo Connect is disabled, but the running server could not stop its tunnel: ${String(liveResult.cause)}\nRestart that server to stop the connector.`,
    );
  } else {
  ...
  if (options.clearAuthorization) {
    yield* Console.log("Signed out of Kairo Connect locally.");
  }
});
  ...
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        const tokens = yield* CliTokenManager.CloudCliTokenManager;
        yield* tokens.get;
        yield* Console.log("Signed in to Kairo Connect.");
      }),
    ),
  ),
);
  ...
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        ...
        yield* Console.log(
          "This Kairo environment will be available through Kairo Connect the next time Kairo starts.",
        );
      }),
    ),
  ),
...
export const connectCommand = Command.make("connect").pipe(
  Command.withDescription("Manage headless Kairo Connect access."),
