...
const status = {
  ...
  installed: true,
  current: true,
  unitPath: "/home/me/.config/systemd/user/kairo.service",
  logPath: "/home/me/.kairo/userdata/logs/boot-service.log",
} as const;
...
it("reports the installed service version and host paths", () => {
  assert.equal(
    ...
    [
      "Kairo service",
      "  Status: installed · kairo@0.0.29",
      "  Unit: /home/me/.config/systemd/user/kairo.service",
      "  Logs: /home/me/.kairo/userdata/logs/boot-service.log",
    ].join("\n"),
  );
});
...
it("gives a direct repair command for a stale service", () => {
  assert.include(
    ...
    "Next: Run `npx kairo@0.0.29 service update`.",
  );
});
...
it("explains an incomplete nightly installation and keeps repair on its installed version", () => {
  ...
  expect(output).toContain("npx kairo@0.0.32-nightly.1 service update");
  expect(output).not.toContain("kairo@latest");
});
...
it("suggests the newer CLI version when the installed service needs an update", () => {
  ...
  expect(output).toContain("npx kairo@0.0.29 service update");
  expect(output).not.toContain("npx kairo@0.0.28 service update");
});
...
it("reports a newer installed service and gives an exact-version repair command", () => {
  ...
  assert.include(output, "kairo@0.0.32-nightly.1 (newer than this kairo@0.0.31 CLI)");
  assert.include(output, "npx kairo@0.0.32-nightly.1 service update");
  assert.notInclude(output, "npx kairo@latest service update");
});
  ...
  it.effect.each(["install", "update"] as const)(
    ...
    (command) =>
      Effect.gen(function* () {
        ...
        const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "kairo-service-cli-test-" });
