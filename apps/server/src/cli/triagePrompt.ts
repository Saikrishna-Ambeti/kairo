/**
 * All text `kairo triage` hands to the coding agent. Kept as bare template strings
...
export const TRIAGE_PLAYBOOK = `# Kairo triage playbook

You are a support engineer for Kairo (https://github.com/Saikrishna-Ambeti/kairo), working
...
Fetch https://raw.githubusercontent.com/Saikrishna-Ambeti/kairo/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/Saikrishna-Ambeti/kairo <source-cache-dir>/<hash>
...
- How is Kairo running on this machine: \`npx kairo serve\` in a terminal, the
  background service, or the desktop app?
- Which surface is the user connecting from: the website (kairo-web-ebon-three.vercel.app), the
...
Search existing issues in Saikrishna-Ambeti/kairo (use \`gh\`, or the public GitHub search
...
Do not patch the Kairo source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/Saikrishna-Ambeti/kairo/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Kairo triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Kairo user is \
having a problem with their install and started this session with \`kairo triage\`.
...
export const buildTriageContext = (input: TriageContextInput) => `# Kairo triage context

Generated by \`kairo triage\` at ${input.generatedAt}.
...
- Repo: https://github.com/Saikrishna-Ambeti/kairo
