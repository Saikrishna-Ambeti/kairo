...
import { dedupeRemoteBranchesWithLocalMatches, normalizeGitRemoteUrl } from "@kairo/shared/git";
import { compactTraceAttributes } from "@kairo/shared/observability";
import { decodeJsonResult } from "@kairo/shared/schemaJson";
...
> {
  ...
  const traceFilePath = yield* fs.makeTempFileScoped({
    prefix: `kairo-git-trace2-${process.pid}-`,
    suffix: ".json",
  });
    ...
    Effect.fn("refreshCheckedOutBranch")(function* (input) {
      ...
      if (!isAncestor) {
        ...
        yield* executeGit(
          ...
          input.cwd,
          ["update-ref", "refs/kairo/pre-refresh", headCommit],
