...
import { VcsDriverKind, type VcsDriverKind as VcsDriverKindType } from "@kairo/contracts";
import { fromLenientJson } from "@kairo/shared/schemaJson";
...
export const make = Effect.gen(function* () {
  ...
  const findConfigPath = Effect.fn("VcsProjectConfig.findConfigPath")(function* (cwd: string) {
    let current = cwd;
    while (true) {
      const candidate = path.join(current, ".kairo", "vcs.json");
