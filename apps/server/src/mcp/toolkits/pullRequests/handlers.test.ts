...
function makeLink(
  ...
  return {
    ...
    number,
    url: `https://github.com/kairo/kairo/pull/${number}`,
    source: "manual",
...
describe("pull request toolkit handlers", () => {
  ...
  it.effect("links by URL with source agent on the token's thread", () =>
    Effect.gen(function* () {
      ...
      const result = yield* harness.call("link_pull_request", {
        url: "https://github.com/Kairo/Kairo/pull/123/files",
      });
  ...
  it.effect("links by repository and number, defaulting the host to the project's", () =>
    Effect.gen(function* () {
      ...
      expect(result).toEqual({
        ...
        repository: "kairo/other",
        number: 7,
        url: "https://github.com/kairo/other/pull/7",
        alreadyLinked: false,
      });
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      const unknown = yield* harness
        .call("link_pull_request", {
          url: "https://github.com/kairo/kairo/issues/1?token=private-value",
        })
        .pipe(Effect.flip);
  ...
  it.effect("treats a duplicate link as alreadyLinked rather than an error", () =>
    Effect.gen(function* () {
      ...
      const result = yield* harness.call("link_pull_request", {
        url: "https://github.com/kairo/kairo/pull/123",
      });
  ...
  it.effect("unlinks a linked pull request and reports a missing one as wasLinked=false", () =>
    Effect.gen(function* () {
      ...
      const missing = yield* harness.call("unlink_pull_request", {
        url: "https://github.com/kairo/kairo/pull/9",
      });
  ...
  it.effect("lists visible links with host state and derived chain order", () =>
    Effect.gen(function* () {
      ...
      expect(result.pullRequests[0]).toEqual({
        ...
        repository: "kairo/kairo",
        number: 3,
        url: "https://github.com/kairo/kairo/pull/3",
        source: "agent",
        state: "open",
        title: "PR 3",
        headBranch: "feat-c",
...
describe("listThreadPullRequests", () => {
  it("reports a native stack position for each member", () => {
    const stack = {
      ...
      id: "stack-1",
      number: 1,
      url: "https://github.com/kairo/kairo/stack/1",
      base: "main",
      layers: [
