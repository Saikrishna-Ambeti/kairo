...
import { AuthStandardClientScopes, EnvironmentId } from "@kairo/contracts";
...
const makeServerConfigLayer = (overrides?: Partial<ServerConfig.ServerConfig["Service"]>) =>
  Layer.effect(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "kairo-auth-session-test-" })),
  );
...
it.layer(NodeServices.layer)("SessionStore.layer", (it) => {
  it.effect("keys remote cookies by environment identity instead of state directory", () =>
    Effect.gen(function* () {
      ...
      const original = yield* cookieName("/srv/kairo-one", EnvironmentId.make("environment-one"));
      const moved = yield* cookieName("/srv/kairo-moved", EnvironmentId.make("environment-one"));
      const other = yield* cookieName("/srv/kairo-one", EnvironmentId.make("environment-two"));
