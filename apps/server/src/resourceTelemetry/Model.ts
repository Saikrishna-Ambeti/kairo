...
export interface TelemetryCounters {
  ...
  readonly allKairo: GroupCounters;
}
...
export interface MergeProcessesResult {
  ...
  readonly groups: {
    ...
    readonly allKairo: ResourceTelemetryAggregate;
  };
...
function applyLifecycleCounters(input: {
  ...
  let allKairo = input.counters.allKairo;
  for (const processDelta of input.deltas) {
    ...
    allKairo = incrementCounters(allKairo, processDelta);
  }
  ...
  for (const [identityKey, current] of input.current) {
    ...
    allKairo = incrementCounters(allKairo, { processStarts: 1 });
  }
  ...
  for (const [identityKey, previous] of input.previous) {
    ...
    allKairo = incrementCounters(allKairo, { processExits: 1 });
  }

  return { backend, electron, monitor, allKairo };
}
...
export function mergeProcesses(input: MergeProcessesInput): MergeProcessesResult {
  ...
  return {
    ...
    groups: {
      ...
      allKairo: aggregate(ordered, counters.allKairo),
    },
    deltas: processDeltas,
  };
}
