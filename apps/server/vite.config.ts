...
export default mergeConfig(
  ...
  defineConfig({
    ...
    pack: {
      ...
      define: {
        __KAIRO_BUILD_CHANNEL__: JSON.stringify(cliBuildChannel),
        __KAIRO_BUILD_RELAY_URL__: JSON.stringify(repoEnv.KAIRO_RELAY_URL?.trim() ?? ""),
        __KAIRO_BUILD_CLERK_PUBLISHABLE_KEY__: JSON.stringify(
          repoEnv.KAIRO_CLERK_PUBLISHABLE_KEY?.trim() ?? "",
        ),
        __KAIRO_BUILD_CLERK_CLI_OAUTH_CLIENT_ID__: JSON.stringify(
          repoEnv.KAIRO_CLERK_CLI_OAUTH_CLIENT_ID?.trim() ?? "",
        ),
        __KAIRO_BUILD_RELAY_CLIENT_OTLP_TRACES_URL__: JSON.stringify(
          repoEnv.KAIRO_RELAY_CLIENT_OTLP_TRACES_URL?.trim() ?? "",
        ),
        __KAIRO_BUILD_RELAY_CLIENT_OTLP_TRACES_DATASET__: JSON.stringify(
          repoEnv.KAIRO_RELAY_CLIENT_OTLP_TRACES_DATASET?.trim() ?? "",
        ),
        __KAIRO_BUILD_RELAY_CLIENT_OTLP_TRACES_TOKEN__: JSON.stringify(
          repoEnv.KAIRO_RELAY_CLIENT_OTLP_TRACES_TOKEN?.trim() ?? "",
        ),
      },
    },
    test: {
