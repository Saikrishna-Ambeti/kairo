...
it("activates only a window of the process authenticated by D-Bus", () => {
  const target = window(42, "Kairo");
  expect(findCaptureDestination([window(99, "Kairo"), target], 42, "Kairo")).toBe(target);
  expect(findCaptureDestination([window(99, "Kairo")], 42, "Kairo")).toBeUndefined();
...
it("does not guess between ambiguous windows from the same process", () => {
  expect(findCaptureDestination([window(42, "Kairo"), window(42, "Kairo")], 42, "Kairo")).toBeUndefined();
