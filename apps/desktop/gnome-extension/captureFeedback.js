...
export class CaptureFeedback {
  begin(sender, pid, snapshot, options) {
    ...
    try {
      ...
      this._clearActors();
      console.warn(`Kairo capture effects unavailable: ${error.message}`);
    }
  ...
  async activate(sender, title) {
    ...
    // A command-palette capture temporarily unmaps Kairo. Wait for its new surface, not a sleep.
    const target =
      find() ??
    ...
    if (this._owned(sender) !== session || !target)
      throw new Error("Kairo's window is not available for activation.");
