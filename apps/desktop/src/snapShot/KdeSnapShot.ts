...
import type { DesktopCaptureHelperState } from "@kairo/contracts";
...
export const KDE_CAPTURE_EXECUTABLE = "kairo-kde-snap-shot";
const DESKTOP_FILE = "com.kairo.Kairo.KdeCapture.desktop";
const MARKER = "X-Kairo-Capture-Helper=true";
...
export function kdeCapturePaths(paths: KdeCapturePaths) {
  return {
    executable: NodePath.join(paths.dataHome, "kairo", "kde-capture", KDE_CAPTURE_EXECUTABLE),
...
export class KdeCaptureSetup {
  ...
  async state(): Promise<DesktopCaptureHelperState> {
    try {
      ...
      if (!bundle)
        return {
          status: "error",
          message: "The capture helper is missing from this build. Update or reinstall Kairo.",
        };
  ...
  async perform(action: "install-kde-helper" | "remove-kde-helper") {
    ...
    if (action === "remove-kde-helper") {
      ...
      if (!bundle)
        throw new Error(
          "The capture helper is missing from this build. Update or reinstall Kairo.",
        );
...
export async function captureKdeWindow(
  ...
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "kairo-kde-capture-"));
  let retained = false;
      ...
      feedback: {
        ...
        activate: async (title) => {
          ...
          const activation = await NodeFSP.mkdtemp(
            NodePath.join(NodeOS.tmpdir(), "kairo-kde-activate-"),
          );
          try {
            await run(
              executable,
