...
/** Niri owns the keybinding; this endpoint triggers capture without first focusing Kairo. */
export async function startNiriCaptureShortcut(
  ...
  try {
    ...
    if (result !== RequestNameReply.PRIMARY_OWNER)
      throw new Error("Another Kairo instance already owns the capture shortcut.");
    return close;
  } catch (error) {
    close();
    throw error;
