...
import type { SnapShotKeyChord } from "@kairo/contracts";
...
export class PortalCaptureShortcut {
  ...
  private failed = (error: unknown) => {
    ...
    this.update({
      ...
      shortcutMessage: this.managedByHyprland
        ? "Couldn't connect to Hyprland shortcuts. Make sure xdg-desktop-portal-hyprland is running, then restart Kairo."
          ...
          this.update({
            ...
            shortcutMessage:
              this.version >= 2
                ...
                : "Shortcut permission wasn't granted. Allow Kairo in your desktop's shortcut settings.",
          });
          return undefined;
        }
    ...
    if (this.managedByHyprland) {
      this.update({
        ...
        shortcutMessage: shortcut
          ...
          : "Hyprland did not register the capture action. Check that xdg-desktop-portal-hyprland is running, then restart Kairo.",
      });
      return;
    }
  ...
  private async initialize(appId: string, shortcut: SnapShotKeyChord) {
    ...
    this.shortcutId = this.managedByHyprland
      ...
      : `kairo-snap-shot-${NodeCrypto.createHash("sha256").update(trigger).digest("hex").slice(0, 16)}`;
