...
it.runIf(hasDbus)("captures through real D-Bus marshalling on a private bus", async () => {
  const directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "kairo-private-dbus-"));
  ...
  try {
    ...
    await server.requestName("org.gnome.Shell.Extensions.KairoSnapShot", NameFlag.DO_NOT_QUEUE);
    ...
    expect(await portal.backend("com.kairo.Kairo")).toBe("screenshot-portal");
    expect(await portal.capturePortal()).toEqual({ png });
    expect(target).toBe(8);
    portalVersion = 2;
    const extension = connect();
    expect(await extension.backend("com.kairo.Kairo")).toBe("gnome-extension");
    expect(await extension.captureExtension("com.kairo.Kairo")).toMatchObject({
      png,
      window: { processId: 42 },
    });
    ...
    expect(await updated.backend("com.kairo.Kairo")).toBe("gnome-extension");
    const snapshot = await updated.captureExtension("com.kairo.Kairo", {
      flash: true,
      animate: true,
    });
    ...
    expect(activateTitle).toBe("Kairo");
    expect(animateFrame).toEqual([0.1, 0.8, 0.2, 0.1]);
    vi.stubEnv("XDG_CURRENT_DESKTOP", "KDE");
    const kde = connect();
    expect(await kde.backend("com.kairo.Kairo")).toBe("kde");
    ...
    const command = [
      ...
      "--session",
      "--dest",
      "com.kairo.Kairo.NiriTest.SnapShot",
      "--object-path",
      "/com/kairo/SnapShot",
      "--method",
      "com.kairo.SnapShot.Capture",
    ];
    ...
    const invalid = server.call(
      new Message({
        destination: "com.kairo.Kairo.NiriTest.SnapShot",
        path: "/com/kairo/SnapShot",
        interface: "com.kairo.SnapShot",
        member: "Capture",
        signature: "s",
        body: ["not allowed"],
      }),
