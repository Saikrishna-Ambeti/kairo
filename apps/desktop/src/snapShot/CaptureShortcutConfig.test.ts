...
// oxlint-disable-next-line kairo/no-global-process-runtime -- Test collection checks the host before starting these Linux-native filesystem tests.
...
const appId = "com.kairo.Kairo";
const install = { operation: "install", chooseFile: false } as const;
const target = () => ({ desktop: "niri" as const, path, appId });
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "kairo-capture-config-"));
