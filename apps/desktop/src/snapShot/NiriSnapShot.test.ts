...
beforeEach(async () => {
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "kairo-niri-test-"));
...
it("selects the native adapter without needing a portal or GNOME extension", async () => {
  expect(await getLinuxCaptureSupport("com.kairo.Kairo")).toEqual({
    linuxBackend: "niri",
    linuxFeedbackAvailable: false,
  });
  const snapshot = await captureLinuxWindow("com.kairo.Kairo");
...
it("does not activate Kairo until requested, then matches PID and title", async () => {
  ...
  windows = [
    { ...window, id: 1, pid: 999, title: "Kairo" },
    { ...window, id: 2, pid: process.pid, title: "Other Kairo" },
    { ...window, id: 3, pid: process.pid, title: "Kairo" },
  ];
  await snapshot.feedback!.activate("Kairo");
  expect(calls).toContainEqual({ Action: { FocusWindow: { id: 3 } } });
});

it("waits for the restored Kairo window to map instead of polling", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  ...
  handler = async (request, socket) => {
    ...
    if (request === "EventStream")
      send(socket, {
        WindowOpenedOrChanged: { window: { ...window, id: 4, pid: process.pid, title: "Kairo" } },
      });
  };
  await snapshot.feedback!.activate("Kairo");
...
it("rejects ambiguous activation targets", async () => {
  const snapshot = await captureNiriWindow(socketPath);
  windows = [1, 2].map((id) => ({ ...window, id, pid: process.pid, title: "Kairo" }));
  await expect(snapshot.feedback!.activate("Kairo")).rejects.toThrow("More than one");
});
...
it("cancels pending activation when capture feedback is closed", async () => {
  ...
  const activation = expect(snapshot.feedback!.activate("Kairo")).rejects.toThrow("cancelled");
...
it("rejects compositor errors and cleans up its temporary image", async () => {
  ...
  await expect(captureLinuxWindow("com.kairo.Kairo")).rejects.toThrow("window disappeared");
  expect(await NodeFSP.stat(NodePath.dirname(capturePath!)).catch(() => undefined)).toBeUndefined();
});
