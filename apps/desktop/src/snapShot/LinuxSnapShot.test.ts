...
beforeEach(async () => {
  ...
  directory = await NodeFSP.mkdtemp(NodePath.join(NodeOS.tmpdir(), "kairo-portal-test-"));
...
it("retains the authenticated connection until the compositor flight has landed", async () => {
  ...
  expect(
    bus.calls
      ...
      .map((call) => call.body),
  ).toEqual([[false, true], ["Kairo"], [0.1, 0.8, 0.2, 0.1]]);
  expect(bus.disconnect).toHaveBeenCalledOnce();
  feedback.close();
  expect(bus.disconnect).toHaveBeenCalledOnce();
});
...
it("can activate without animation, and expires a renderer-abandoned capture", async () => {
  ...
  await expect(snapshot!.feedback!.activate("Kairo")).rejects.toThrow("No window");
