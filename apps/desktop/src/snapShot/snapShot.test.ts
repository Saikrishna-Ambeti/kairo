...
describe("findAccessibleWindow", () => {
  ...
  it.each(["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"])(
    ...
    (frame) => {
      const windows = [{ name: `${frame} kairo`, bounds: captured.bounds }];

      expect(findAccessibleWindow(windows, { ...captured, title: "⠋ kairo" }, "wayland")).toBe(
        windows[0],
      );
    },
  );
  ...
  it("rejects matching spinners when the window sizes differ", () => {
    expect(
      findAccessibleWindow(
        [{ name: "⠙ kairo", bounds: { ...captured.bounds, width: 400 } }],
        { ...captured, title: "⠋ kairo" },
        "wayland",
      ),
    ).toBeUndefined();
  });
  ...
  it("rejects ambiguous normalized titles even if one matches the captured spinner exactly", () => {
    const windows = [
      { name: "⠋ kairo", bounds: captured.bounds },
      { name: "⠙ kairo", bounds: captured.bounds },
    ];

    expect(
      findAccessibleWindow(windows, { ...captured, title: "⠋ kairo" }, "wayland"),
    ).toBeUndefined();
  });

  it("keeps exact title matching outside Wayland", () => {
    expect(
      findAccessibleWindow([{ name: "⠙ kairo", bounds: captured.bounds }], {
        ...captured,
        title: "⠋ kairo",
      }),
    ).toBeUndefined();
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(async () => {
          ...
          const runtimeDirectory = await mkdtemp(join(tmpdir(), "kairo-wayland-"));
          ...
          try {
            ...
            expect(isWaylandSession("linux", { XDG_RUNTIME_DIR: "/nonexistent-kairo-test" })).toBe(
              false,
            );
          } finally {
            if (server.listening) {
