...
it.effect.each(["win32", "darwin", "linux"] as const)(
  ...
  (platform) => {
    ...
    const kairo = {
      id: 42,
      title: "Kairo",
      appIdentifier: "com.kairo.Kairo.desktop",
      owner: { name: "Kairo", processId: 123 },
      bounds,
      png: Buffer.from([1, 2, 3]),
    };
    focusedWindowMock.mockReturnValue({
      getBounds: () => bounds,
      getTitle: () => kairo.title,
      isDestroyed: () => false,
      isMinimized: () => false,
      isVisible: () => true,
      show: vi.fn(),
    ...
    macCaptureMock.mockReset().mockImplementation(async () => {
      images.push(kairo.png);
      return { source: { name: kairo.title }, png: kairo.png };
    });
    ...
    linuxCaptureMock.mockResolvedValueOnce({
      ...
      window: {
        title: kairo.title,
        appName: kairo.owner.name,
        appIdentifier: kairo.appIdentifier,
        processId: kairo.owner.processId,
        bounds,
      },
      feedback: {
    ...
    return Effect.scoped(
      Effect.gen(function* () {
        ...
        const saved = yield* decodePendingMetadata(metadata);
        assert.equal(saved.source.windowTitle, kairo.title);
        assert.equal(saved.source.appName, kairo.owner.name);
        assert.equal(saved.source.accessibleText, `Window from process ${kairo.owner.processId}`);
        assert.deepEqual(images, [kairo.png]);
        assert.equal(prepareCaptureRevealMock.mock.calls.length, platform === "win32" ? 1 : 0);
        if (platform === "linux") {
          assert.equal(saved.source.appIdentifier, kairo.appIdentifier);
          assert.deepEqual(activate.mock.calls, [[kairo.title]]);
        }
      }),
    ).pipe(
      Effect.provide(
      ...
      Effect.gen(function* () {
        ...
        const warning = logs.find(
          (message) =>
            ...
            message[0] === "The compositor could not activate Kairo after the snapshot",
        );
...
it.each([
  { names: ["⠙ kairo"], expected: "Verified text" },
  { names: ["⠋ kairo", "⠙ kairo"], expected: undefined },
...
it.effect("flags revoked macOS permissions on read and re-registers once they return", () => {
  ...
  return Effect.scoped(
    Effect.gen(function* () {
      ...
      assert.equal(
        revoked.message,
        "Allow Screen Recording in System Settings, then restart Kairo.",
      );
...
for (const fails of [false, true]) {
  it.effect(`tests macOS capture without publishing it and cleans up, failure=${fails}`, () => {
    const active = {
      ...
      id: 42,
      title: "Setup",
      owner: { name: "Kairo", processId: 123, path: "/Applications/Kairo.app" },
      bounds: { x: 0, y: 0, width: 800, height: 600 },
    };
    activeWindowMock.mockReset().mockResolvedValue(active);
