...
describe("ElectronWindow", () => {
    ...
    Effect.gen(function* () {
      ...
      const options = {
        ...
        webPreferences: {
          ...
          partition: "persist:kairo-preview-test",
          sandbox: true,
      ...
      assert.equal(error.message, 'Failed to create Electron BrowserWindow "Kairo" (1100x780).');
  ...
  it.effect("focuses the exact Kairo window before activating from a shell-hosted app", () =>
    Effect.gen(function* () {
      const operations: Array<string> = [];
  ...
  it.effect("prepares the exact Kairo window before a capture overlay", () =>
    Effect.gen(function* () {
