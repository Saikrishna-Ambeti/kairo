...
const OVERLAY_ATTRIBUTE = "data-kairo-annotation-ui";
const Z_INDEX_OVERLAY = 2147483646;
const PRIMARY = "var(--kairo-primary)";
const PRIMARY_FILL = "color-mix(in srgb, var(--kairo-primary) 10%, transparent)";
...
const applyAnnotationTheme = (
  ...
  const variables = {
    "--kairo-radius": theme.radius,
    "--kairo-background": theme.background,
    "--kairo-foreground": theme.foreground,
    "--kairo-popover": theme.popover,
    "--kairo-popover-foreground": theme.popoverForeground,
    "--kairo-primary": theme.primary,
    "--kairo-primary-foreground": theme.primaryForeground,
    "--kairo-muted": theme.muted,
    "--kairo-muted-foreground": theme.mutedForeground,
    "--kairo-accent": theme.accent,
    "--kairo-accent-foreground": theme.accentForeground,
    "--kairo-border": theme.border,
    "--kairo-input": theme.input,
    "--kairo-ring": theme.ring,
    "--kairo-font-sans": theme.fontSans,
    "--kairo-font-mono": theme.fontMono,
  };
...
function startAnnotation(): void {
  ...
  const refreshToolButtons = (): void => {
    ...
    document.documentElement.setAttribute("data-kairo-annotation-tool", tool);
  };
      ...
      if (isUsableRect(rect)) {
        ...
        if (found === 0) {
          ...
          const regionBox = createBox(
            PRIMARY,
            "color-mix(in srgb, var(--kairo-primary) 6%, transparent)",
          );
  ...
  const teardown = (notifyMain: boolean): void => {
    ...
    document.documentElement.removeAttribute("data-kairo-annotation-tool");
    cursorStyle.remove();
    host.remove();
    activeSession = null;
