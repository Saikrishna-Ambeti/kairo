...
import * as SshAuth from "@kairo/ssh/auth";
import { resolveSshTarget } from "@kairo/ssh/command";
import { discoverSshHosts } from "@kairo/ssh/config";
import {
  SshCommandError,
  ...
  SshReadinessError,
} from "@kairo/ssh/errors";
import * as SshTunnel from "@kairo/ssh/tunnel";
...
>()("@kairo/desktop/ssh/DesktopSshEnvironment") {}

export interface DesktopSshEnvironmentLayerOptions {
  readonly resolveCliPackageSpec?: () => string;
  readonly resolveCliRunner?: Effect.Effect<SshTunnel.RemoteKairoRunnerOptions>;
}
...
export function toSshPasswordPromptError(
  ...
  switch (cause._tag) {
    ...
    case "DesktopSshPromptPresentationError":
      message = "Kairo window is not available for SSH authentication.";
      break;
