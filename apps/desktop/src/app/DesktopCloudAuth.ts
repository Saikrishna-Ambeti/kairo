...
export const CLOUD_AUTH_CALLBACK_STATE_PARAM = "kairo_state";
export const CLOUD_AUTH_CALLBACK_SCHEME = "kairo";
export const DEVELOPMENT_CLOUD_AUTH_CALLBACK_SCHEME = "kairo-dev";
...
export class DesktopCloudAuth extends Context.Service<DesktopCloudAuth, DesktopCloudAuthShape>()(
  "@kairo/desktop/app/DesktopCloudAuth",
) {}
...
function resolveConfiguredProtocolClient(): {
  ...
  const path = process.env.KAIRO_DESKTOP_PROTOCOL_CLIENT_PATH?.trim();
  if (!path) return null;

  return {
    path,
    args: (process.env.KAIRO_DESKTOP_PROTOCOL_CLIENT_ARGS ?? "")
      .split("\n")
      .map((arg) => arg.trim())
...
function isProtocolRegistrationManagedExternally(): boolean {
  return process.env.KAIRO_DESKTOP_PROTOCOL_REGISTRATION_MANAGED?.trim() === "1";
}

function resolveProtocolCallbackForwardUrl(): URL | null {
  const rawUrl = process.env.KAIRO_DESKTOP_PROTOCOL_CALLBACK_URL?.trim();
