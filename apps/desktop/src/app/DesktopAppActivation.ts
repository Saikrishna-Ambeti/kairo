...
import { HostProcessUserId } from "@kairo/shared/hostProcess";
  ...
  const server = NodeNet.createServer((socket) => {
    ...
    socket.on("data", (chunk) => {
      ...
      void input.handle(parsed).then(finish, () => {
        finish(
          invalidResponse(parsed.requestId, "Kairo could not process the desktop app request."),
        );
      });
    });
...
>()("@kairo/desktop/app/DesktopAppActivation") {}
