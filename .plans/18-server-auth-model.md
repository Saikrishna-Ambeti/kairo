...
Define the long-term server auth architecture for Kairo Code before first-class remote environments ship.
...
Every privileged surface of the Kairo server must go through the same auth policy engine:
...
Participants:
  ...
  KairoServer      = local backend child process
...
DesktopMain -> KairoServer : spawn server (--bootstrap-fd ...)
DesktopMain -> KairoServer : send desktop bootstrap envelope
note over KairoServer : policy = DesktopManagedLocalPolicy
note over KairoServer : allowed pairing = desktop-bootstrap only

Frontend -> DesktopMain : request local bootstrap grant
DesktopMain --> Frontend : short-lived desktop bootstrap grant

Frontend -> KairoServer : POST /api/auth/bootstrap
KairoServer -> KairoServer : validate desktop bootstrap grant
KairoServer -> KairoServer : create browser session
KairoServer --> Frontend : Set-Cookie: session=...

Frontend -> KairoServer : GET /ws + authenticated cookie
KairoServer -> KairoServer : validate cookie session
KairoServer --> Frontend : websocket accepted
```
...
```text
Participants:
  UserShell     = npx kairo launcher
  KairoServer      = standalone local server
  Browser       = browser tab

UserShell -> KairoServer : start server
KairoServer -> KairoServer : getOrCreate("server-signing-key")
note over KairoServer : policy = LoopbackBrowserPolicy

UserShell -> KairoServer : issue one-time pairing token
KairoServer --> UserShell : pairing URL or pairing token

UserShell --> Browser : open /pair?token=...

Browser -> KairoServer : GET /pair?token=...
KairoServer -> KairoServer : validate one-time token
KairoServer -> KairoServer : create browser session
KairoServer --> Browser : Set-Cookie: session=...
KairoServer --> Browser : redirect to app

Browser -> KairoServer : GET /ws + authenticated cookie
KairoServer --> Browser : websocket accepted
```
...
Participants:
  ...
  KairoServer      = Kairo server
  PhoneBrowser  = mobile browser

DesktopUser -> DesktopMain : enable remote access via tunnel
DesktopMain -> KairoServer : switch policy to RemoteReachablePolicy
DesktopMain -> Tunnel : publish local Kairo endpoint
Tunnel --> DesktopMain : public https/wss URL

DesktopMain -> KairoServer : issue one-time pairing token
KairoServer --> DesktopMain : pairing token
...
Tunnel -> KairoServer : forward request
KairoServer -> KairoServer : validate one-time token
KairoServer -> KairoServer : create mobile browser session
KairoServer --> PhoneBrowser : Set-Cookie: session=...
KairoServer --> PhoneBrowser : redirect to app

PhoneBrowser -> Tunnel : GET /ws + authenticated cookie
Tunnel -> KairoServer : forward websocket upgrade
KairoServer --> PhoneBrowser : websocket accepted
```
...
DesktopUser -> KairoServer : enable private-network access
KairoServer -> KairoServer : switch policy to RemoteReachablePolicy
DesktopUser -> KairoServer : issue one-time pairing token
KairoServer --> DesktopUser : pairing URL / QR

DesktopUser -> PhoneBrowser : open private-network URL
PhoneBrowser -> PrivateNet : GET http(s)://private-host/pair?token=...
PrivateNet -> KairoServer : route request
KairoServer -> KairoServer : validate one-time token
KairoServer -> KairoServer : create mobile browser session
KairoServer --> PhoneBrowser : Set-Cookie: session=...
KairoServer --> PhoneBrowser : redirect to app
...
PrivateNet -> KairoServer : websocket upgrade
KairoServer --> PhoneBrowser : websocket accepted
```
...
Participants:
  ...
  RemoteKairo      = remote Kairo server
  Frontend      = desktop renderer

DesktopUser -> DesktopMain : add SSH host
DesktopMain -> SSH : connect to remote host
SSH -> RemoteHost : probe environment / verify kairo availability
DesktopMain -> SSH : run remote launch command
SSH -> RemoteHost : kairo remote launch --json
RemoteHost -> RemoteKairo : start or reuse server
RemoteKairo --> RemoteHost : port + environment metadata
...
note over RemoteKairo : policy = RemoteReachablePolicy
note over DesktopMain,RemoteKairo : desktop may use a trusted bootstrap flow here

Frontend -> DesktopMain : request bootstrap for selected environment
DesktopMain --> Frontend : short-lived bootstrap grant

Frontend -> RemoteKairo : POST /api/auth/bootstrap via forwarded port
RemoteKairo -> RemoteKairo : validate bootstrap grant
RemoteKairo -> RemoteKairo : create browser session
RemoteKairo --> Frontend : Set-Cookie: session=...

Frontend -> RemoteKairo : GET /ws + authenticated cookie
RemoteKairo --> Frontend : websocket accepted
```
