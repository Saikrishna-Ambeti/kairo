...
- users can expose a backend through LAN, their own Tailscale, MagicDNS, a manual HTTPS endpoint, or later Kairo Tunnel
- users can generate a hosted pairing link for `app.kairo.codes`
...
This plan intentionally leaves the paid Kairo cloud tunnel fabric out of scope. It defines the OSS foundation that Kairo Tunnel should later plug into.
...
- `https://app.kairo.codes` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.kairo.codes` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.kairo.codes/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.kairo.codes`.
...
- `app.kairo.codes` works as a real client shell.
...
- `app.kairo.codes` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.kairo.codes`.
...
## Phase 4: Future Kairo Tunnel Provider
...
- create endpoint with `provider: "kairo-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Kairo Code should only see an `AdvertisedEndpoint`.
