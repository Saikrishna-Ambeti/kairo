...
pub fn destination(windows: Vec<Window>, pid: u32, title: &str) -> Result<Option<Window>> {
    ...
    if matches.next().is_some() {
        return Err("More than one Kairo window matches the capture destination.".into());
    }
    Ok(found)
}

pub fn activate(pid: u32, title: &str) -> Result<()> {
    // Subscribe before looking up a newly mapped Kairo window, so no map/title event is missed.
    ...
    loop {
        ...
        if let Some(window) = destination(windows()?, pid, title)? {
            ...
            {
                return Err("Hyprland could not focus Kairo.".into());
            }
            return Ok(());
        }
        events.get_ref().set_read_timeout(Some(
            deadline
                ...
                .ok_or("Kairo did not become visible.")?,
        ))?;
...
mod tests {
    ...
    fn destination_requires_unique_process_and_title() {
        assert!(
            destination(vec![window(2, "Kairo")], 1, "Kairo")
                .unwrap()
                .is_none()
        );
        assert!(
            destination(vec![window(1, "other")], 1, "Kairo")
                .unwrap()
                .is_none()
        );
        assert!(destination(vec![window(1, "Kairo"), window(1, "Kairo")], 1, "Kairo").is_err());
        assert!(
            destination(vec![window(2, "Kairo"), window(1, "Kairo")], 1, "Kairo")
                .unwrap()
                .is_some()
        );
    }
