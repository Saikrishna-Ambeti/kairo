...
const OBJECT: &str = "/com/kairo/KdeCapture/Feedback";
...
#[zbus::interface(name = "com.kairo.KdeCapture.Feedback")]
impl Bridge {
...
pub(super) fn run(connection: &Connection, directory: &Path, options: &str) -> Result<()> {
    ...
    let name = format!("kairo-capture-feedback-{}", std::process::id());
