...
test("renders baseline, impact, ceiling, and ceiling changes", () => {
  ...
  const comment = renderComment({
    ...
    currentRun: {
      ...
      conclusion: "success",
      url: "https://github.com/pingdotgg/kairo/actions/runs/2",
    },
    baselineRun: {
      sha: "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
      matchesBase: true,
      url: "https://github.com/pingdotgg/kairo/actions/runs/1",
    },
  });
  ...
  assert.match(comment, /<!-- kairo-thread-transfer-report -->/);
  assert.match(
    comment,
    /<!-- kairo-thread-transfer-result-sha:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb -->/,
  );
});
...
test("resolves a fallback PR with a redacted head repo and exact main baseline", async () => {
  ...
  await resolve({
    github,
    context: {
      repo: { owner: "pingdotgg", repo: "kairo" },
      payload: {
        workflow_run: {
          id: 2,
          event: "pull_request",
          ...
          head_repository: { full_name: "pingdotgg/kairo" },
          conclusion: "success",
          pull_requests: [],
        },
      },
      ...
      paginate: async (method) => {
        ...
        return [5350, 5351].map((number) => ({
          ...
          head: {
            sha: "head-sha",
            ref: "feature-branch",
            repo: { full_name: "pingdotgg/kairo" },
          },
        }));
      },
      rest: {
      ...
      paginate: async () => [
        {
          ...
          body: `<!-- kairo-thread-transfer-report -->\n<!-- kairo-thread-transfer-result-sha:${sha} -->`,
        },
      ],
      rest: {
        issues: {
