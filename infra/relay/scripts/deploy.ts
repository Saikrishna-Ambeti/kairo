...
const publicConfigEnvEntries = (config: RelayPublicConfig) =>
  ({
    ...
    KAIRO_MOBILE_OTLP_TRACES_URL: config.mobileTracingUrl,
    KAIRO_MOBILE_OTLP_TRACES_DATASET: config.mobileTracingDataset,
    KAIRO_MOBILE_OTLP_TRACES_TOKEN: config.mobileTracingToken,
    KAIRO_RELAY_CLIENT_OTLP_TRACES_URL: config.clientTracingUrl,
    KAIRO_RELAY_CLIENT_OTLP_TRACES_DATASET: config.clientTracingDataset,
    KAIRO_RELAY_CLIENT_OTLP_TRACES_TOKEN: config.clientTracingToken,
  }) as const;
...
export function reconcileRootEnvRelayUrl(contents: string, relayUrl: string): string {
  return reconcileRootEnvPublicConfig(contents, {
    ...
    .split("\n")
    .filter((line) => !line.startsWith("KAIRO_MOBILE_OTLP_TRACES_"))
    .filter((line) => !line.startsWith("KAIRO_RELAY_CLIENT_OTLP_TRACES_"))
    .join("\n");
}
...
const readRelayPublicConfig = Effect.fn("relay.deploy.readState")(function* (stage: string) {
  ...
  const output = yield* service.getOutput({ stack: "KairoRelay", stage });
...
export const relayDeployCommand = Command.make(
  ...
  deploy,
).pipe(Command.withDescription("Deploy the Kairo relay through Alchemy."));
