...
import { resolveGitWorktreePath, resolveWorktreeKairoHome } from "./devHome.ts";
...
const makeRepo = (
  ...
  Effect.acquireRelease(
    Effect.sync(() => {
      const root = NodeFS.mkdtempSync(NodePath.join(NodeOS.tmpdir(), "kairo-devhome-"));
      if (kind === "worktree") {
...
describe("resolveWorktreeKairoHome", () => {
  it.effect("answers with .kairo before the dev runner creates it", () =>
    Effect.gen(function* () {
      const { root, nested } = yield* makeRepo("worktree");
      const home = yield* resolveWorktreeKairoHome(nested);
      assert.equal(home, NodePath.join(NodePath.resolve(root), ".kairo"));
