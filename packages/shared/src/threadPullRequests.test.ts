...
function link(
  ...
  return {
    ...
    number,
    url: `https://github.com/pingdotgg/kairo/pull/${number}`,
    source: "manual",
...
describe("threadPullRequestKeysEqual", () => {
  it("ignores host and repository case", () => {
    expect(
      threadPullRequestKeysEqual(
        { host: "GitHub.com", repository: "PingDotGG/kairo", number: 1 },
        { host: "github.com", repository: "pingdotgg/kairo", number: 1 },
      ),
    ).toBe(true);
    expect(
      threadPullRequestKeysEqual(
        { host: "github.com", repository: "pingdotgg/kairo", number: 1 },
        { host: "gitlab.com", repository: "pingdotgg/kairo", number: 1 },
      ),
    ).toBe(false);
  });
});
...
describe("resolveThreadCurrentPullRequest", () => {
  ...
  it("reports a stack when several links are open and puts the highest layer on top", () => {
    const stack = {
      ...
      id: "s1",
      number: 1,
      url: "https://github.com/pingdotgg/kairo/stacks/1",
      base: "main",
      layers: [
...
describe("legacyLinkedPullRequestOf", () => {
  const identity = {
    ...
    provider: "github",
    displayName: "pingdotgg/kairo",
    locator: {
      source: "git-remote" as const,
      remoteName: "origin",
      remoteUrl: "https://github.com/pingdotgg/kairo.git",
    },
  };
  it("projects only links the owning project can route without a host", () => {
    expect(legacyLinkedPullRequestOf([link(7)], "project-1" as never, identity)).toEqual({
      ...
      repository: "pingdotgg/kairo",
      number: 7,
      url: "https://github.com/pingdotgg/kairo/pull/7",
    });
  ...
  it.each([
    {
      ...
      url: "https://github.enterprise.test/pingdotgg/kairo/pull/7",
    },
...
describe("threadPullRequestSearchTerms", () => {
  it("includes completed and unsynced links but excludes dismissed links", () => {
    ...
    expect(terms).toContain("pingdotgg/kairo#12");
    expect(terms).toContain("https://github.com/pingdotgg/kairo/pull/12");
...
it("searches the legacy projection when old environments decode to an empty links list", () => {
  const linkedPullRequest = {
    ...
    repository: "pingdotgg/kairo",
    number: 12,
    url: "https://github.com/pingdotgg/kairo/pull/12",
  };
