 ...
 * The state directory is hashed so custom Kairo homes cannot exceed Unix socket
 * path limits.
 */
export function resolveDesktopAppControlAddress(input: {
  ...
  if (input.platform === "win32") {
    return {
      address: `\\\\.\\pipe\\kairo-app-${stateHash}`,
      directory: null,
    };
  }
  ...
  const directory = input.joinPath(input.tempDir, `kairo-${userKey}`);
  return {
