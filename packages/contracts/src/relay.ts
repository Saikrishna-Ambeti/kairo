...
}).annotate({ description: "Links an authenticated cloud user to a Kairo environment." });
...
>()("@kairo/contracts/relay/RelayClientPrincipal") {}
...
>()("@kairo/contracts/relay/RelayEnvironmentPrincipal") {}

const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Kairo Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["kairo-mobile", "kairo-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "kairo-mobile" as const;
export const RelayWebClientId = "kairo-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Kairo Relay API")
  .annotate(OpenApi.Version, "1.0.0")
  .annotate(
    OpenApi.Description,
    "Control-plane API for linking Kairo environments, connecting authorized clients, and publishing agent activity.",
  );
