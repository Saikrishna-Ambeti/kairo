...
describe("resolveMediaSource", () => {
  describe("direct URLs", () => {
    ...
    it.each(["data:image/png;base64,AAAA", "blob:https://app.kairo.codes/id"])(
      "loads %s directly",
      (href) => {
        expect(resolveMediaSource(href, { threadId, imageEmbed: true })).toMatchObject({
          access: "direct",
  ...
  it("serves Kairo attachment files in place like any other host path", () => {
    const path = `/home/demo/.kairo/userdata/attachments/${attachmentId}.mp4`;
